    ...
    if (!empty($class_methods)) {
        if (is_array($value)) {
            foreach ($class_methods as $fct_name) {
                if (substr($fct_name, 0, 3) == "set") {
                    ...
                    if (isset($value[$rskey])) {
                        call_user_func(array($obj, "set" . $rskey), $value[$rskey]);
                    }
                }
            }
            ...
            foreach ($class_methods as $fct_name) {
                if (substr($fct_name, 0, 3) == "set") {
                    $rskey = camelcase2underscore(substr($fct_name, 3));
                    if (!is_null($gvalue = @call_user_func(array($value, "get" . $rskey)))) {
                        call_user_func(array($obj, "set" . $rskey), $gvalue);
                    }
                }
            }
        }
...
function get_inclusion($src_list, $inc_list = array(), $src_key, $inc_key = "")
{
    ...
    if (is_object($inc_list)) {
        foreach ($inc_list as $obj) {
            $tmplist[] = call_user_func(array($obj, "get" . $inc_key));
        }
        $inc_list = $tmplist;
    }
    ...
    foreach ($src_list as $obj) {
        ...
        if (is_array($src_key)) {
            foreach ($src_key as $rs_key) {
                $ar_src_value[] = call_user_func(array($obj, "get" . $rs_key));
            }
            $src_value = @implode("::", $ar_src_value);
        } else {
            $src_value = call_user_func(array($obj, "get" . $src_key));
        }
        ...
        foreach ($class_methods as $fct_name) {
            if (substr($fct_name, 0, 3) == "get") {
                ...
                if (!(strpos($rskey, "create_") === 0 || strpos($rskey, "modify_") === 0)) {
                    $rs_ar[] = $rskey . "=" . htmlentities(addslashes(@call_user_func(array($obj, "get" . $rskey))), ENT_QUOTES, 'UTF-8');
                }
            }
        }
