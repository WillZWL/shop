...
$this->tbswrapper->tbsLoadTemplate('resources/template/search.html', '', '', $data['lang_text']);

if($data["parent_cat_url"])
{
	$parent_cat_url = $data["parent_cat_url"].'?w='.$data["keyword"].'&page='.$data["curr_page"].'&rpp='.$data["rpp"].'&sort='.$data["sort"];
}
/*
...
foreach($data["product_list"] AS $key=>$value)
{
	...
	$prod_list[$key]["stock_status"] =  $value["stock_status"];
...
if($range["start"] && $range["end"])
{
	...
	for($i = $range["start"]; $i <= $range["end"]; $i++)
	{
		...
		$page[$j]["page_no"] = $i;
		$page[$j]["url"] = "?w=".$data["keyword"]."&page=".$i."&rpp=".$data["rpp"]."&sort=".$data["sort"];
		$j++;
	}
}
if($range["prev"])
{
	$prev_page = "?w=".$data["keyword"]."&page=".$range["prev"]."&rpp=".$data["rpp"]."&sort=".$data["sort"];
}
if($range["next"])
{
	$next_page = "?w=".$data["keyword"]."&page=".$range["next"]."&rpp=".$data["rpp"]."&sort=".$data["sort"];
}

foreach($data["brand_result"] AS $key=>$arr)
{
	$brand[$key]["url"] = $_SERVER['REDIRECT_URL'].'?w='.$data["keyword"].'&page=1&rpp='.$data["rpp"].'&sort='.$data["sort"]."&brand_id=".$arr["id"];
...
$sort_arr = array("price_asc"=>$data['lang_text']['price_low_to_high'], "price_desc"=>$data['lang_text']['price_high_to_low'], "latest_asc"=>$data['lang_text']['new_arrival']);
...
if($data["brand_id"])
{
	$brand_url = $_SERVER['REDIRECT_URL'].'?w='.$data["keyword"].'&page='.$data["curr_page"].'&rpp='.$data["rpp"].'&sort='.$data["sort"];
}
