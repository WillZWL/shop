...
$this->tbswrapper->tbsLoadTemplate('resources/template/myaccount_rma_edit_'.get_lang_id().'.html');
...
foreach($cat_arr as $k=>$v)
{
	$category[$k]["id"] = $k;
	$category[$k]["name"] = $v;
	$category[$k]["selected"] = $data["rma_obj"]->get_category() == $k?"SELECTED":"";
}
...
foreach($reason_arr as $k=>$v)
{
	$reason[$k]["id"] = $k;
	$reason[$k]["name"] = $v;
	$reason[$k]["selected"] = $data["rma_obj"]->get_reason() == $k?"SELECTED":"";
}

foreach($action_arr as $k=>$v)
{
	$action_request[$k]["id"] = $k;
	$action_request[$k]["name"] = $v;
	$action_request[$k]["selected"] = $data["rma_obj"]->get_action_request() == $k?"SELECTED":"";
}
