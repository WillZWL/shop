...
?>
    ...
    <tr>
        ...
        <td width="30%" class="value"><input type="checkbox" name="is_advertised[<?=$platform?>]" <?=($price_obj->get_is_advertised()=='Y'?"CHECKED":"")?>></td>
    </tr>
    <tr>
        <td width="20%" class="field"><?=$lang["listing_status"]?></td>
        <td colspan="3" class="value"><select name="listing_status[<?=$platform?>]" style="width:130px;"><option value="N"><?=$lang["not_listed"]?></option><option value="L" <?=($price_obj->get_listing_status() == "L"?"SELECTED":"")?>><?=$lang["listed"]?></option></select><input type="hidden" name="formtype[<?=$platform?>]" value="<?=$formtype[$platform]?>"></td>
    </tr>
    </table>
    </td>
</tr>
...
TOTAL COST =  VAT + DUTY + CHARGE + ADMIN FEE + FREIGHT TO WH + FREIGHT TO FC + POSTAGE + SUPP.COST; <br>
