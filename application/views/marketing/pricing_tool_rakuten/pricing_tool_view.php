...
?>
<tr class="header">
	<td height="20" align="left" style="padding-left:8px;"><b style="font-size: 12px; color: rgb(255, 255, 255);"><span style="padding-right:15px;" id='sign_<?=$platform?>'>-</span><?=$platform_id." - ".$value["obj"]->get_platform_name()." | ".$value["obj"]->get_platform_currency_id()." | "?>
...
?>
	...
	<tr>
		...
		<td colspan="3" class="value">
			<input name="price_ext[<?=$platform?>][title]" value="<?=$price_ext_obj->get_title()?htmlspecialchars($price_ext_obj->get_title()):htmlspecialchars($prod_name_lang)?>" class="input" maxlength="255">
			<br><?=(trim($detail_desc)=="")?"<font color='red'><b>Detail Desc is empty!</b></font>":"";?>
		</td>
	</tr>
	<tr>
		<td class="field" width="20%">Description (Max 15,000 char)<br><font color='red'><b>*DO NOT LEAVE THIS EMPTY</b></font></td>
		<td colspan="3" class="value">
			<textarea name="price_ext[<?=$platform?>][ext_desc]" required="1" maxlength="15000" cols="150"><?=htmlspecialchars($ext_desc)?></textarea>
		</td>
	</tr>
	<tr>
...
html;
			...
			echo $gtin_html;
?>
			<input name="price_ext[<?=$platform?>][gtin][val]" class="input" type="text" value="<?=$gtin_val?$gtin_val:""?>" style="width:180px;" maxlength="17">
...
<?php
			if($store_cat_list[$platform])
			{
				...
				{
?>
				<option value="<?=$store_cat->get_ext_id()?>" <?=($price_ext_obj->get_ext_ref_4() == $store_cat->get_ext_id()?"SELECTED":"")?>><?=$store_cat->get_ext_name()?></option>
<?php
				}
			}
?>
		...
		<td width="20%" class="field">Listing Qty <br>limitPurchaseQuantity <br>Rakuten Shop Inventory</td>
		<td width="30%" class="value">
			<input name="price_ext[<?=$platform?>][ext_qty]" class="input" type="text" value="<?=$price_ext_obj->get_ext_qty()==""?"":$price_ext_obj->get_ext_qty()?>" style="width:80px;"> (-1 = unlimited)<br>
			<input name="price_ext[<?=$platform?>][ext_ref_2]" class="input" type="text" value="<?=$price_ext_obj->get_ext_ref_2()==""?"":$price_ext_obj->get_ext_ref_2()?>" style="width:80px;"> (-1 = unlimited) <br>
...
?>
	<tr>
		...
		<td class="value">
			<select id="action[<?=$platform?>]"  name="action[<?=$platform?>]" style="width:130px;">
				<option></option>
...
?>
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tb_main">
		<col width="15%"><col width="8%"><col width="8%"><col width="10%"><col width="10%"><col width="12%"><col width="15%">
		<tr>
...
<tr>
	...
	<tr >
		...
		<td width="30%" class="value"><select name="status" style="width:130px;" onChange="lockqty(this.value)">
<?php
		foreach( $ar_ws_status as $key=>$value)
		{
?>
					<option value="<?=$key?>" <?=($prod_obj->get_website_status() == $key?"SELECTED":"")?>><?=$value?></option>
<?php
		}
?>
		</select></td>
...
?>
			<div><?=$lang["create_note"]?></div><div><input name="m_note" maxlength="255" type="text"></div>
		</td>
		<td width="20%" class="field" valign="top" style="padding-top:2px; padding-bottom:2px;"><?=$lang["sourcing_notes"]?></td>
		<td width="30%" class="value" valign="top" style="padding-top:2px; padding-bottom:2px;"><div><?=$lang["current_notes"]?></div>
<?php
...
<?php
		...
		?>
			<div><?=$lang["create_note"]?></div><div><input name="s_note" maxlength="255" type="text"></div>
		</td>
	</tr>
	<?="<!--"?>
