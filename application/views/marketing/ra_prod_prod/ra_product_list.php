...
?>
    <label for='rprod<?=$i?>_<?=$ele?>' class="div_select_lable"><input name="rprod<?=$i?>[]" value='<?=$pobj->get_sku()?>' type='checkbox' id='rprod<?=$i?>_<?=$ele?>' onclick='highlight_div(this);'><span style="width:480px;">[<?=$pobj->get_sku()?>] <?=$pobj->get_prod_name()?></span><span style="width:100px"><?=$lang["status"]?>: <?=$ar_ws_status[$pobj->get_website_status()]?></span><span style="width:120px"><?=$lang["profit"]?>: <?=$currency?> <?=number_format($pobj->get_profit(),2)?></span><span style="width:100px"><?=$lang["margin"]?>: <?=$currency?> <?=number_format($pobj->get_margin(),2)?></span></label>
<?php
                    if ($ra_prods[$pobj->get_sku()])
                    {
?>
                        <script>document.getElementById('rprod<?=$i?>_<?=$ele?>').checked=true;highlight_div(document.getElementById('rprod<?=$i?>_<?=$ele?>'));</script>
<?php
                        $got = 1;
                    }
                    $ele++;
...
?>
    ...
    <td id="status_<?=$i?>" class="value" width="100" nowrap style="white-space: nowrap">&nbsp;</td>
    <td id="profit_<?=$i?>" class="value" width="100" nowrap style="white-space: nowrap">&nbsp;</td>
