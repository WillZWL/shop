...
<div class="ui-widget">
	...
	<form action="<?=base_url()?>marketing/ext_category_mapping" method="GET">
		...
		<select id="gcat_country" name="gcat_country" style="width:100px;size:30">
			...
			{?>
				<option id="<?=$country?>" <?=$_GET["gcat_country"]==$country?" selected":""?> ><?=$country?></option>
			<?}?>
		</select>
		<br>
	...
	<?foreach($cat_details_list[1] as $cat_obj) {?>
		<option id="<?=$cat_obj->get_id()?>" parent_cat_id = "<?=$cat_obj->get_parent_cat_id()?>" ><?=$cat_obj->get_name()?></option>
	<?}?>
	</select>
	...
	<?foreach($google_category_list as $google_category) {?>
		<option country_id="<?=$google_category->get_country_id()?>" value="<?=$google_category->get_id()?>"><?=htmlspecialchars($google_category->get_ext_name())?></option>
	<?}?>
	</select>
</div>
