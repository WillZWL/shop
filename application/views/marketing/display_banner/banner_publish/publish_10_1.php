...
<?php
    ...
    if ($link_type[0] == "L") {
        if (file_exists(VIEWPATH . "banner/lytebox_" . $publish_key . ".php") == FALSE) {
            ?>
            <a href="<?= "http://" . $lytebox_country_id . ".chatandvision.com/checkout/lytebox/" . $publish_key ?>"
               onclick="return false;" rel="lyteframe[checkout]"
        ...
        <?php
            ...
            <a href="<?= base_url() . "checkout/lytebox/" . $publish_key ?>" onclick="return false;"
               rel="lyteframe[checkout]" rev="width: 620px; height:410px; scrolling: auto;padding: 40px;"
               title="<?= $lang["lytebox_title"] ?>"><img width="<?= $banner_width ?>" height="<?= $banner_height ?>"
                                                          border="0" src="<?= $graphic[0] ?>"></a>
        <?php
        }
