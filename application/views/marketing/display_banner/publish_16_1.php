    ...
    if ($link_type[0] == "L") {
        if (file_exists(VIEWPATH . "banner/lytebox_" . $publish_key . ".php") == FALSE) {
            ...
            <a href="<?= "http://" . $lytebox_country_id . ".chatandvision.com/header/lytebox/" . $publish_key ?>"
               ...
               target="lbIframe" title="<?= $lang["lytebox_title"] ?>"><img width="<?= $banner_width ?>"
                                                                            height="<?= $banner_height ?>" border="0"
                                                                            src="<?= $graphic[0] ?>"></a>
        <?php
        } else {
            ?>
            <link rel="stylesheet" href="/css/lytebox.css" type="text/css" media="screen"/>
            <a href="<?= base_url() . "header/lytebox/" . $publish_key ?>" rel="lyteframe[header]"
