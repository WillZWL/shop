    ...
    <div id="main">
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
                ...
                <td width="400" align="right" class="title"><input type="button" value="<?=$lang["content_manage"]?>" class="button" onclick="Redirect('<?=site_url('marketing/customer_service_info/index/')?>')"></td>
            </tr>
            <tr>
        ...
        <form action="" method="POST" name="tform" style="padding:0; margin:0" onSubmit="return CheckForm(this)" enctype="multipart/form-data">
            ...
            <col width="10%"><col width="30%"><!--<col width="15%"><col width="15%">--><col width="15%"><!--<col width="15%">-->
            <tr class="header">
...
-->
                <td style="text-align:center;color:#F0F0F0"><input type="button" value="Select All" style="font-size:11px" onClick="javascript:selectAll('st_status_w')">&nbsp;&nbsp;&nbsp;<input type="button" value="Deselect All" style="font-size:11px" onclick="deSelectAll('st_status_w')"></td>
<!--
...
-->
                <td style="text-align:center"><input type="checkbox" name="st_status_we[<?=$lang_key?>]" onClick="selectAllLang('st_status_w\\[<?=$lang_key?>\\]', this.checked)" <?=$st_status_we[$lang_key]?"CHECKED":""?>/></td>
<!--
...
-->
            ...
            <tr class="row<?=$i%2?>" onMouseOver="AddClassName(this, 'highlight')" onMouseOut="RemoveClassName(this, 'highlight')">
                ...
                <td><?=$obj->get_name()?></td>
<!--
                <td style="text-align:center"><input type="checkbox" name="st_status_s[<?=$lang_key?>][<?=$obj->get_id()?>]" <?php if($pbv['SKYPE'][$lang_key][$obj->get_id()] == 1){echo $st_status_s[$obj->get_id()]?"CHECKED":"";}else{echo "DISABLED";} ?>/></td>
                <td style="text-align:center"><input type="checkbox" name="lt_status_s[<?=$lang_key?>][<?=$obj->get_id()?>]" <?php if($pbv['SKYPE'][$lang_key][$obj->get_id()] == 1){echo $lt_status_s[$obj->get_id()]?"CHECKED":"";}else{echo "DISABLED";} ?>/></td>
-->
                <td style="text-align:center"><input type="checkbox" name="st_status_w[<?=$lang_key?>][<?=$obj->get_id()?>]" <?php if($pbv['WEBSITE'][$lang_key][$obj->get_id()] == 1){echo $st_status_w[$obj->get_id()]?"CHECKED":"";}else{echo "DISABLED";} ?>/></td>
<!--
                <td style="text-align:center"><input type="checkbox" name="lt_status_w[<?=$lang_key?>][<?=$obj->get_id()?>]" <?php if($pbv['WEBSITE'][$lang_key][$obj->get_id()] == 1){echo $lt_status_w[$obj->get_id()]?"CHECKED":"";}else{echo "DISABLED";} ?>/></td>
-->
            </tr>
            <?php
                    $i++;
