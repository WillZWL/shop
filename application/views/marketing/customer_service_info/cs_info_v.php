...
<div id="main">
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tr>
            ...
            <td width="400" align="right" class="title"><input type="button" value="<?= $lang["status_manage"] ?>"
                                                               class="button"
                                                               onclick="Redirect('<?= site_url('marketing/customer_service_info/status') ?>')">
            </td>
        </tr>
        <tr>
                    ...
                    <table align="center">
                        <tr>
                            <td style="border:0px">
                                ...
                                <select
                                    onChange='gotoPage("<?= base_url() . "marketing/customer_service_info/index/" ?>",this.value)'>
                                    <option value=""> -- <?= $lang["please_select"] ?> --</option><?php
                                    foreach ($lang_list as $lang_obj) {
                                        ?>
                                        <option
                                        value="<?= $lang_obj->get_id() ?>" <?= ($lang_obj->get_id() == $language_id ? "SELECTED" : "") ?>><?= $lang_obj->get_name() ?></option><?php
                                    }
                                    ?>
                                </select>
                            </td>
                            ...
                            <td style="border:0px">
                                <?= $lang["select_country"] ?>:
                                <select
                                    onChange='SaveChange(this);gotoPage("<?= base_url() . "marketing/customer_service_info/index/ALL/" ?>",this.value)'>
                                    <option value=""> -- <?= $lang["please_select"] ?> --</option><?php
                                    foreach ($country_list as $obj) {
                                        ?>
                                        <option
                                        value="<?= $obj->get_id() ?>" <?= ($obj->get_id() == $country_id ? "SELECTED" : "") ?>><?= $obj->get_name() ?></option><?php
                                    }
                                    ?>
                                </select>
                            </td>
