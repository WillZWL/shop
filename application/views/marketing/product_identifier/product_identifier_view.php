...
<tr class="header">
	<td height="20" align="left" style="padding-left:8px;"><b style="font-size:price_obj 12px; color: rgb(255, 255, 255);"><?=$lang["product_info"]?></b></td>
</tr>
<tr>
	<td width="100%">
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="tb_main">
	<col width="15%"><col width="23%"><col width="23%"><col width="23%"><col width="10%">
	<tr>
		...
		<td class="field" align="centre" style="text-align:center"><b><?=$lang["status"]?></b><br /><input type="button" value="Select" style="font-size:10px;width:40%" onClick="javascript:selectAll('status')">&nbsp;<input type="button" value="Deselect" style="font-size:10px;width:50%" onclick="deSelectAll('status')"></td>
	</tr>
<?php
if($country_list)
{
...
?>
	<tr>
		...
		<td class="row<?=$i%2?>"><input type="text" class="input" name="ean[<?=$curr_country_id?>]" value="<?=$ean?>"></td>
		<td class="row<?=$i%2?>"><input type="text" class="input" name="mpn[<?=$curr_country_id?>]" value="<?=$mpn?>"></td>
		<td class="row<?=$i%2?>"><input type="text" class="input" name="upc[<?=$curr_country_id?>]" value="<?=$upc?>"></td>
		<td class="row<?=$i%2?>" align="center"><input type="checkbox" name="status[<?=$curr_country_id?>]" value="1" <?=$status?"checked":""?>><input type="hidden" name="country_id[<?=$curr_country_id?>]" value="<?=$curr_country_id?>"></td>
	</tr>
<?php
		$i++;
	}
...
<tr>
	<td align="right" style="padding-right:8px;" height="30"><input type="button" value="Update" class="button" onClick="if(CheckForm(this.form)) this.form.submit();"></td>
</tr>
</table>
</form>
<?php
