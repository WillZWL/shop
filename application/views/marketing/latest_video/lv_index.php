...
<div id="main">
        ...
        <table border="0" cellpadding="0" cellspacing="0" height="70" class="page_header" width="100%">
            ...
            <tr>
                ...
                <td colspan=4 class="value value_text">&nbsp;&nbsp;
                    <select
                        onChange='SaveChange(this);gotoPage("<?= base_url() . "marketing/latest_video/main/?catid=" . $this->input->get('catid') . "&level=" . $this->input->get('level') . "&type=" . $this->input->get('type') . "&src=" . $this->input->get('src') . "&platform=" ?>",this.value)'>
            ...
            <tr>
                ...
                <td colspan=4 class="value value_text">&nbsp;&nbsp;
                    <select
                        onChange='SaveChange(this);gotoPage("<?= base_url() . "marketing/latest_video/main/?catid=" . $this->input->get('catid') . "&level=" . $this->input->get('level') . "&platform=" . $this->input->get('platform') . "&type=" . $this->input->get('type') . "&src=" ?>",this.value)' <?= $this->input->get('platform') ? "" : "disabled" ?>>
            ...
            <tr>
                ...
                <td colspan=4 class="value value_text">&nbsp;&nbsp;
                    <select
                        onChange='SaveChange(this);gotoPage("<?= base_url() . "marketing/latest_video/main/?catid=" . $this->input->get('catid') . "&level=" . $this->input->get('level') . "&platform=" . $this->input->get('platform') . "&src=" . $this->input->get('src') . "&type=" ?>",this.value)' <?= $this->input->get('src') ? "" : "disabled" ?>>
                        <option value=""> -- Please select --</option>
    ...
    <?php  if ($display) {
        ?>
        <iframe
            src="<?= base_url() ?>marketing/latest_video/view_left/?cat=<?= $catid ?>&level=<?= $level ?>&platform=<?= $platform ?>&type=<?= $type ?>&src=<?= $src ?>"
        ...
        <iframe
            src="<?= base_url() ?>marketing/latest_video/view_right/<?= $catid ?>/<?= $platform ?>/<?= $type ?>/<?= $src ?>"
