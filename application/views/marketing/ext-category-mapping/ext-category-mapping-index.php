...
body{
	...
	#selectable li { margin: 3px; padding: 0.4em; font-size: 15px; height: 12px; }
	#select-result
	{
		display:block;
		margin: 10px;
...
<div class="ui-widget">
	<br>------------------<br>
	<form action="<?=base_url()?>marketing/ext-category-mapping" method="GET">
		...
		<select id="gcat_country" name="gcat_country" style="width:100px;size:30">
			...
			<?php foreach($country_list as $country) :?>
				<option id="<?=$country["platform_country_id"]?>" <?=$_GET["gcat_country"]==$country["platform_country_id"]?" selected":""?>><?=$country["platform_country_id"]?></option>
			<?php endforeach?>
		</select>
		<br>
	...
	<?php foreach($cat_details_list[0] as $catObj):?>
		<option id="<?=$catObj->getId()?>"><?=$catObj->getName()?></option>
	<?php endforeach?>
	</select>
	...
	<option id="" value=""></option>
	<?php foreach($cat_details_list[1] as $catObj):?>
		<option id="<?=$catObj->getId()?>" parent_cat_id = "<?=$catObj->getParentCatId()?>" ><?=$catObj->getName()?></option>
	<?php endforeach?>
	</select>
	...
	<option id="" value=""></option>
	<?php foreach($cat_details_list[2] as $catObj):?>
		<option id="<?=$catObj->getId()?>" parent_cat_id = "<?=$catObj->getParentCatId()?>" ><?=$catObj->getName()?></option>
	<?php endforeach?>
	</select>
	...
	<option id="" value=""></option>
	<?php foreach($country_list as $country):?>
		<option id="<?=$country["platform_country_id"]?>"><?=$country["platform_country_id"]?></option>
	<?php endforeach?>
	</select>
	...
	<option id="" value=""></option>
	<?php foreach($google_category_list as $googleCategory):?>
		<option country_id="<?=$googleCategory->getCountryId()?>" value="<?=$googleCategory->getId()?>"><?=htmlspecialchars($googleCategory->getExtName())?></option>
	<?php endforeach?>
	</select>
</div>
...
$(function(){
	...
	$("#ad_start").click(function(){
		...
		$.ajax({
			url:"ext-category-mapping/compaign_info",
			type:"POST",
			dataType:"json",
	...
	$(document).on('click', '.campaign_item', function(){
		...
		$.ajax({
			url:"ext-category-mapping/adgroup_info",
			type:"POST",
	...
	//	$("#dialog_ajax_feedback").dialog("open");
		$.ajax({
			url:"ext-category-mapping/keyword_info",
			type:"POST",
		...
		$.ajax({
			url:"ext-category-mapping/adGroup_ad_info",
			type:"POST",
			data:{"ad_accountId":ad_accountId,"adGroupId":adGroupId},
			dataType:"JSON",
			success:function(data){
	...
	$(document).on("click",".keyword_item",function(){
		...
		$.ajax({
			url: "ext-category-mapping/keyword_parameter_info",
			type:"POST",
			DataType: "html",
			data: {"ad_accountId":ad_accountId,
					"keywordId":keywordId,
