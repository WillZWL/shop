        ...
        if ($objcount) {
                    ...
                    <td align="left">
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tb_main">
                            <tr>
                                ...
                                <td class="value"><input name="price[<?= $platform ?>][platform_code]"
                                                         value="<?= htmlspecialchars($price_obj->get_platform_code()) ?>">
                                </td>
                                <td width="20%" class="field"><?= $lang["fulfillment_centre_id"] ?></td>
                                <td width="30%" class="value">
                                    <select id="price_ext[<?= $platform ?>][fulfillment_centre_id]"
                                            name="price_ext[<?= $platform ?>][fulfillment_centre_id]"
                                            onchange="rePrice('<?= $platform ?>', 4);">
                                        <?php
                            ...
                            <tr>
                                ...
                                <td width="30%" class="value"><select name="price[<?= $platform ?>][auto_price]">
                                        <option
                                            value="N" <?= $price_obj->get_auto_price() == "N" ? "SELECTED" : "" ?>><?= $lang["disabled"] ?></option>
                                        <option
                                            value="Y" <?= $price_obj->get_auto_price() == "Y" ? "SELECTED" : "" ?>><?= $lang["enabled"] ?></option>
                                    </select></td>
                                ...
                                <td width="30%" class="value">
                                    <select name="price_ext[<?= $platform ?>][amazon_reprice_name]">
                                        ...
                                        foreach ($reprice_name_list[$platform] as $rule) {
                                            ...
                                            ?>
                                            <option
                                                value="<?= $rule->get_id(); ?>" <?= $selected ?>><?= $rule->get_id(); ?></option>
                                        <?php
                                        }
                                        ?>
                                    </select>
                                    <input value="<?= $lang['edit_reprice'] ?>"
                                           onClick="window.open('/marketing/ixtens_reprice_rule/','ixtens_reprice_rule','width=1280,height=768')"
                                           type="button">
                                </td>
                            </tr>
                            <tr>
                                <td class="field"><?= $lang["latency_in_stock"] ?><a href="/images/amazon_latency.jpg"
                                                                                     target="_blink"><font color="blue"
                                                                                                           style="font-size:10px">
                                            &nbsp;&nbsp;Show Detail</font></a></td>
                                <td class="value"><input name="price[<?= $platform ?>][latency]"
                                                         value="<?= htmlspecialchars($price_obj->get_latency()) ?>">
                                </td>
                                <td class="field"><?= $lang["latency_out_stock"] ?><a href="/images/amazon_latency.jpg"
                                                                                      target="_blink"><font color="blue"
                                                                                                            style="font-size:10px">
                                            &nbsp;&nbsp;Show Detail</font></a></td>
                                <td class="value"><input name="price[<?= $platform ?>][oos_latency]"
                                                         value="<?= htmlspecialchars($price_obj->get_oos_latency()) ?>">
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="value">
                                    <select name="price_ext[<?= $platform ?>][ext_condition]">
                                        ...
                                        foreach ($condition_list as $condition) {
                                            ...
                                            ?>
                                            <option
                                                value="<?= $condition->get_id() ?>" <?= $selected ?>><?= $condition->get_condition(); ?></option>
                                        <?php
                                        }
                                        ?>
                                    </select>
                                ...
                                <td class="value"><input name="price[<?= $platform ?>][max_order_qty]"
                                                         value="<?= htmlspecialchars($price_obj->get_max_order_qty()) ?>">
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="value">
                                    <select name="listing_status[<?= $platform ?>]" style="width:130px;">
                                        ...
                                        <option
                                            value="NC" <?= ($price_obj->get_listing_status() == "NC" ? "SELECTED" : "") ?>><?= $lang["no_content"] ?></option>
                                        <option
                                            value="NS" <?= ($price_obj->get_listing_status() == "NS" ? "SELECTED" : "") ?>><?= $lang["unsuitable"] ?></option>
                                    </select>
            ...
            if ($canedit) {
                ...
                <div style="margin-top:20px; margin-top:20px; text-align:left; padding-left:10px;">
                    ...
                    DUTY = DECL. * DUTY% <br>
                    PAYPAL FEE = SELLING PRICE * PAYPAL FEE% + <!--<script>document.write(paypal_fee_adj)</script>-->
                    <br>
                    TOTAL COST = TOTAL COST = VAT + DUTY + EBAY LISTING FEE + EBAY COMMISSION + PAYPAL FEE + ADMIN FEE +
                    FREIGHT TO WH + FREIGHT TO FC + POSTAGE + SUPP.COST; <br>
