...
<head>
    ...
    <script type="text/javascript" src="<?= base_url() ?>marketing/pricing_tool_amfr/get_js"></script>
</head>
...
<div id="main" style="width:auto">
    ...
    if ($value != "") {
        if ($canedit) {
            ?>
            <form name="list" action="<?= base_url() ?>marketing/pricing_tool_amfr/view/<?= $prod_obj->get_sku() . ($this->input->get('target') == "" ? "" : "?target=" . $this->input->get('target')) ?>" method="POST" onSubmit="return CheckForm(this);">
        ...
        if ($price_obj->get_platform_code() != "") {
            $link = "http://www.amazon.fr/dp/" . $price_obj->get_platform_code();
            $tar = "target='websitefr'";
        }
        ?>
<table border="0" cellpadding="0" cellspacing="0" width="100%">
<tr>
    ...
    <div style="float:right"><img src='<?= base_url() ?>/images/<?= $lang["platform_country_image"] ?>' height="70"> &nbsp;</div>
        ...
        if ($objcount) {
            ...
            <tr class="header">
                <td height="20" align="left" style="padding-left:8px;"><b
                        style="font-size: 12px; color: rgb(255, 255, 255);"><?= $lang["pricing_info"] . "<br>" . $lang["listing_currency"] . $currency_obj->get_id() . " - " . $currency_obj->get_name() ?></b>
                </td>
            </tr>
            <tr>
                <td align="left">
            ...
            if ($price_obj->get_listing_status() != "L") {
                ...
                <tr height="20" bgcolor="ee3333">
                    <td align="center"><font
                            style="font-weight:bold; color:#dddddd;"><?= $lang["currrent_not_listed"] ?></font></td>
                </tr>
            <?php
            } else {
                ?>
                ...
                <td width="100%">
                    <table cellpadding="0" cellspacing="0" border="0" width="100%" class="tb_main">
                        ...
                        <tr>
                            ...
                            <td width="30%" class="value"><input type="text" name="latency"
                                                                 value="<?= $price_obj->get_latency() == "" ? 0 : $price_obj->get_latency() ?>"
                                                                 isNatural min=0></td>
                        </tr>
                        <?php
                        if (count($inv)) {
                        ...
                        <tr>
                            <td width="20%" class="field" valign="top"
                                style="padding-top:2px; padding-bottom:2px;"><?= $lang["listing_status"] ?></td>
                            <td width="30%" class="value" valign="top" style="padding-top:2px; padding-bottom:2px;">
                                <select name="listing_status" style="width:130px;">
                            ...
                            <td width="30%" class="value"><input name="platform_code" type="text"
                                                                 value="<?= $price_obj->get_platform_code() ?>"></td>
                        </tr>
                        <tr>
                            <td width="20%" class="field"><?= $lang["max_order_qty"] ?></td>
                            <td width="30%" class="value"><input type="text" name="max_order_qty"
                                                                 value="<?= $price_obj->get_max_order_qty() == "" ? 100 : $price_obj->get_max_order_qty() ?>">
                            </td>
                            <td width="20%" class="field"><?= $lang["autoprice"] ?></td>
                            <td width="30%" class="value"><select name="auto_price">
                                    <option
                        ...
                        <tr>
                            ...
                            <td width="30%" class="value"><input type="checkbox"
                                                                 name="allow_express" <?= ($price_obj->get_allow_express() == 'Y' ? "CHECKED" : "") ?>>
                            </td>
                            <td width="20%" class="field"><?= $lang["advertised"] ?></td>
                            <td width="30%" class="value"><input type="checkbox"
                                                                 name="is_advertised" <?= ($price_obj->get_is_advertised() == 'Y' ? "CHECKED" : "") ?>>
                            </td>
                        </tr>
                        <tr>
                            <td width="20%" class="field" valign="top"
            ...
            if ($canedit) {
                ...
                <table border="0" cellpadding="0" cellspacing="0" width="100%" class="tb_detail">
                    <tr>
                        <td align="right" style="padding-right:8px;" height="30"><input type="submit"
                ...
                <div style="margin-top:20px; margin-bottom:20px; text-align:left; padding-left:10px;">
                    ...
                    TOTAL COST = VAT + DUTY + CHARGE + ADMIN FEE + AMAZON COMMISSION + FREIGHT + SUPP.COST; <br>
