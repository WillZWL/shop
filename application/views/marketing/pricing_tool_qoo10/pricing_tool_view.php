...
function check_sub_cat_margin(platform)
{
    if(document.getElementById('auto_price_cb['+platform+']').checked == true)
    {
        if(!document.getElementById('sub_cat_margin['+platform+']').value)
        {
            ...
            document.getElementById('auto_price_cb['+platform+']').checked = false;
        }
        else
        {
            document.getElementById('sp['+platform+']').readOnly = true;
            document.getElementById('sp['+platform+']').value = -1;//document.getElementById('auto_calc_price['+platform+']').value;
    ...
    <!-- <td height="20" align="left" style="padding-left:8px;"><b style="font-size: 12px; color: rgb(255, 255, 255);"><a href="javascript:showHide('<?//=$platform?>');"><span style="padding-right:15px;" id='sign_<?//=$platform?>'>+</span></a><?//=$platform_id." - ".$value["obj"]->get_platform_name()." | ".$value["obj"]->get_platform_currency_id()." | "?> -->
            ...
            <?php
                if(($price_ext_obj->get_ext_item_id()))
                {
                    ...
                    {
                        case "QOO10SG":
                            $qoo10_url = "<a href='http://list.qoo10.sg/GMKT.INC/Goods/Goods.aspx?goodscode=".$price_ext_obj->get_ext_item_id()."' target='_blank'>http://list.qoo10.sg/GMKT.INC/Goods/Goods.aspx?goodscode=".$price_ext_obj->get_ext_item_id()."</a>";
                            break;
                    }
                    echo " | ".$qoo10_url;
                }
...
?>
    ...
    <tr>
        <td width="20%" class="field"><?=$lang["qoo10_title"]?></td>
        <td colspan="3" class="value"><input name="price_ext[<?=$platform?>][title]" value="<?=htmlspecialchars($price_ext_obj->get_title())?>" class="input" maxlength="200"></td>
    </tr>
    ...
    <?php
        if($store_cat_list[$platform])
        {
            ...
            {
                ?><option value="<?=$store_cat->get_ext_id()?>" <?=($price_ext_obj->get_ext_ref_3() == $store_cat->get_ext_id()?"SELECTED":"")?>><?=$store_cat->get_ext_name()?></option><?php
            }
        }
    ?>
        </td>
    ...
    <tr>
        ...
        <td class="value">
            <select name="listing_status[<?=$platform?>]" onChange="if(this.value == 'N') {document.getElementById('action[<?=$platform?>]').value ='E'} else {document.getElementById('action[<?=$platform?>]').value =''}" style="width:130px;">
...
?>
    <tr>
        <td class="field"><?=$lang["qoo10_item_id"]?></td>
...
?>
    ...
    <tr >
        ...
        <td width="80%" colspan='3' class="value" valign="top" style="padding-top:2px; padding-bottom:2px;"><input type="text"  name="max_order_qty" value="<?=($price_obj->get_max_order_qty()==""?"100":$price_obj->get_max_order_qty())?>"></td>
    </tr>
    <tr >
        <td width="20%" class="field"><?=$lang["create_on"]?></td>
...
<tr>
    <td align="right" style="padding-right:8px;" height="30"><input type="button" id="update_pricing_tool" value="Update Pricing Tool" class="button" onClick="if(CheckForm(this.form)) this.form.submit();"></td>
</tr>
</table>
</form>
