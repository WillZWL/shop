...
<tr height="20">
    ...
    <td class="value" align="left">&nbsp;&nbsp;<?=$avalue[$i].($aname[$i]==""?"":" - ".$aname[$i])?></td>
    <td class="field" align="right" style="padding-right:5px;"><?=$lang["over_option"]." ".$i?></td>
    <td class="value" align="left">&nbsp;&nbsp;<?=$ovalue[$i].($oname[$i]==""?"":" - ".$oname[$i])?></td>
</tr>
<?php
    }
?>
...
<tr>
    ...
    <td class="value" align="left">&nbsp;&nbsp;<?=$value[$i].($name[$i]==""?"":" - ".$name[$i])?></td>
    <td class="field" align="right" style="padding-right:5px;"><?=$lang["noption"]." ".$i?></td>
    <td class="value" align="left">&nbsp;&nbsp;<input name="cat[<?=$i?>]" type="text" value="" id="cat<?=$i?>" READONLY>&nbsp;&nbsp;<span id="nid<?=$i?>"></span></td>
</tr>
<?php
    }
?>
...
<tr class="header">
    <td width="55%" height="40" style="padding-left:20px;"><input type="button" value="<?=$lang["back"]?>" onClick="parent.document.location.href='<?=$_SESSION["LISTPAGE"]?>';" class="button"></td>
    <td align="left">&nbsp;&nbsp;
    <input type="submit" name="for_all_platform_use" value="For All Platform Use" onclick="return confirm('Do you want to use these products as default for All Platforms?');" title="use these products for ALL platforms" value="1">
    <!-- blow submit's value is used to detect which button is clicked. therefore, $lang['submit'] should not be empty -->
    <input type="submit" name="posted" value="<?=$lang["submit"]?>"  class="button">&nbsp;&nbsp;<input type="reset" value="<?=$lang["reset"]?>" class="button" onClick="count = 1; resetForm();"></td>
</tr>
</table>
