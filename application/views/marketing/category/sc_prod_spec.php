...
<tr>
    <td height="50" style="padding-left:8px"><b style="font-size:14px"><?=$lang["header"]." ".$cat_obj->get_name()?></b>
</tr>
</table>
...
<col><col width="150"><col width="150"><col width="150"><col width="150">
<?php
    if($full_cps_list)
{
    ...
    <tr class="value">
        ...
        <td>
            ...
            else
            {
                ...
                {
                ?>
                 &nbsp;&nbsp<input type="hidden" name="cps_obj[<?=$cps_obj->get_ps_func_id()?>][unit_id]" value="txt">Not suitable
                <?php
                }
                else
                {
            ?>
                <select name="cps_obj[<?=$cps_obj->get_ps_func_id()?>][unit_id]" class="input">
            <?php
                $selected_s[$cps_obj->get_unit_id()] = "SELECTED";
                foreach($ut_array[$cps_obj->get_unit_type_id()] AS $obj)
                {
                ?>
                <option value="<?=$obj->get_id()?>" <?=$selected_s[$cps_obj->get_unit_id()]?>><?=$obj->get_unit_name()?>
                <?php
                }
            ?>
            </select>
        ...
        <td><input name="cps_obj[<?=$cps_obj->get_ps_func_id()?>][priority]" class="input" value="<?=$cps_obj->get_priority()?>" notEmpty isNumber min=0></td>
        <td>
