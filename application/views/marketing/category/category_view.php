...
<?php
	...
	{
?>
div.layer<?=$i?>{padding-left:<?=$step * $j?>px;}
span.aspan{ margin-right:8px;}
<?php
		$j++;
	}
?>
div.layerp{padding-left:<?=$step * $j?>px;}
</style>
...
<tr>
	...
	<td width="400" align="right" class="title"><input type="button" value="Generate Menu" class="button" onClick="document.getElementById('update_menu').src='<?=base_url()?>cron/cron_draw_menu/cron_multilanguage_menu'"></td>
</tr>
<tr>
...
<tr class="header">
	...
	{
	?><td height="20" align="left"  valign="middle" width="155"><input type="button" value="Add sub-category under this" onclick=Redirect('<?=base_url()."marketing/category/add/?level=".($cat_obj->get_level()+1)."&parent=".$cat_obj->get_id()?>') style="width:150px;"></td><?php
	}
	?></font></td>
</tr>
<tr>
...
<tr>
	...
	<td height="20" valign="top" class="value" align="left" colspan="2"><input type="text" name="name" value="<?=$cat_obj->get_name()?>" notEmpty></td>
</tr>
<tr>
	<td width="142" height="20" valign="top" class="field" align="right"><?=$lang["display_content"]?></td>
	<td class="value" align="left" colspan="2">
		...
		<div id="div_image_tabs" style="border:0px;padding:0px;">
			...
			?>
			<div id="div_name_<?=$cur_lang_id?>" class="x-tab" title="<?=$lang_obj->get_name()?>" style="border:0px">
			<table border="0" cellpadding="0" cellspacing="1" height="20" class="page_header" width="100%">
				...
				<tr>
					...
					<td class="value" colspan="2"><input name="lang_name[<?=$cur_lang_id?>]" class="input" value="<?=htmlspecialchars($cur_name)?>"></td>
				</tr>
			<!--
				<tr>
				...
				<tr>
					...
					<td class="value" colspan="2"><textarea cols="100" rows="8" name="latest_news[<?=$cur_lang_id?>]"><?if($cat_cont_obj[$lang_obj->get_id()]){$cat_cont_obj[$lang_obj->get_id()]->get_latest_news();}?></textarea></td>
				</tr>
			-->
			</table>
			</div>
...
<tr>
	...
	<td align="left" class="value" colspan="2"><textarea cols="100" rows="5"   name="description"><?=htmlspecialchars(stripslashes($cat_obj->get_description()))?></textarea></td>
</tr>
<?php
...
<tr>
	...
	<td align="left" class="value" colspan="2">
		<select name="add_colour_name" style="width:120px;">
			<option value="0" <?=($cat_obj->get_add_colour_name()==0?"SELECTED":"")?>><?=$lang["no"]?></option>
			<option value="1" <?=($cat_obj->get_add_colour_name()==1?"SELECTED":"")?>><?=$lang["yes"]?></option>
		</select>
	</td>
</tr>
<?php
...
<?php
	...
	{
?>
	<option value="<?=$i?>" <?=($i == $cat_obj->get_status()?"SELECTED":"")?>><?=$lang["status".$i]?></option>
<?php
	}
?>
	</select></td>
...
<tr>
	...
	<td height="20" valign="top" class="value" align="left" colspan="2"><input type="text" name="priority" value="<?=$cat_obj->get_priority()?>" isInteger min=1></td>
</tr>
<?php
	if($cat_obj->get_level() == 2)
	{
?>
<tr>
	<td width="142" height="20" valign="top" class="field" align="right"><?=$lang["bundle_discount"]?></td>
	<td height="20" valign="top" class="value" align="left" colspan="2"><input type="int_input" name="bundle_discount" value="<?=$cat_obj->get_bundle_discount()?>" class="int_input" min="0"> %</td>
</tr>
<?php
	}
?>
<tr>
						...
						<table width="100%" style="padding-right: 10px;">
							...
							<td>
								<?if($cat_obj->get_level() > 1){?>
									<font style="font-size:10px">P: <?=$parcode[$j]['code']?></font><br>
								<?}?>

								<select style="width: 120px;" name="hscode_<?=$hs[$j]['country_id']?>" id="hscode_<?=$hs[$j]['country_id']?>" onChange="fillduty($(this).val(), $(this).find(':selected').attr('duty'), $(this).find(':selected').attr('cid'))">
									<?php for($i=0; $i<count($optionhs);  $i++){ ?>
										<?php if($optionhs[$i]['country_id'] == $hs[$j]['country_id']) { ?>
											<option value="<?=$optionhs[$i]['code']?>" duty="<?=$optionhs[$i]['duty_pcent']?>" cid="<?=$optionhs[$i]['country_id']?>"<?=($optionhs[$i]['code'] == $hs[$j]['code']?"SELECTED":"")?>><?=$optionhs[$i]['code']?>, <?=$optionhs[$i]['duty_pcent']?>%,  <?=$optionhs[$i]['country_id']?>, <?=$optionhs[$i]['description']?>
											</option>
										<?php } ?>
									<?}?>
								</select>
							...
							<td style="text-align: center">
								<?if($cat_obj->get_level() > 1){?>
									<font style="font-size:10px"><?=$parcode[$j]['duty_pcent']?></font><br>
								<?}?>

							<input type="int_input" name="duty_<?=$hs[$j]['country_id']?>" id="duty_<?=$hs[$j]['country_id']?>" value="<?=$hs[$j]['duty_pcent']?>" class="int_input" style="width:32px" min="0">%</td>
						   </tr>
						</table>
					</td>
				<?php  if($m==3){
						...
						<table width="100%" style="padding-right: 10px;">
							...
							<?if(strpos($psarr, $arrcountry[$j] ) !==false) {?>
										...
										<?if($cat_obj->get_level() > 1){?>
											...
											<font style="font-size:10px">P: <?=$rcode?></font><br>
										<?}?>

										<select style="width: 120px;" name="hscode_<?= $arrcountry[$j]?>" id="hscode_<?= $arrcountry[$j]?>" onChange="fillduty($(this).val(), $(this).find(':selected').attr('duty'), $(this).find(':selected').attr('cid'))">
										<?php for($i=0; $i<count($optionhs);  $i++){ ?>
							...
							<?if(strpos($psarr, $arrcountry[$j] ) !==false) {?>
										...
										<?if($cat_obj->get_level() > 1){?>
											...
											<font style="font-size:10px"><?=$rduty?></font><br>
										<?}?>
										...
										<?if(count($hs)>0) {
											...
											<?php }?>
											<input type="int_input" name="duty_<?=$arrcountry[$j]?>" id="duty_<?=$arrcountry[$j]?>" value="<?=$pduty?>" class="int_input" style="width:32px" disabled min="0">%
											<?//} ?>
										<?php } ?>
									...
									<?php }else{ ?>
										...
										<input type="int_input" name="duty_<?=$arrcountry[$j]?>" id="duty_<?=$arrcountry[$j]?>" value="" class="int_input" style="width:32px" disabled min="0">%
	...
	<?php } ?>
		...
		<?if($cat_obj->get_level() > 1){?>
		<input type="button" name="inherit" value="Inherit Parent" style="font-size:11px; height: 30px; width: 100;text-align: center;" onClick="inheritparent(<?php $cat_obj->get_parent_cat_id() ?>)">
		<?}?>
		</td>
	</tr>
	</table>
...
<tr>
	...
	<td height="20" valign="top" class="value" align="left" colspan="2"><select name="subcat" style="width:300px;"><?php
			foreach($parent_list as $pobj)
			{
?><option value="<?=$pobj->get_id()?>" <?=($pobj->get_id() == $cat_obj->get_parent_cat_id()?"SELECTED":"")?>><?=$pobj->get_name()?></option><?php
			}
	?></select></td>
</tr>
<?php
...
<tr>
	<td width="142" height="20" valign="top" class="field" align="right"><?=$lang["category_cat"]?></td>
	<td height="20" valign="top" class="value" align="left" colspan="2"><?=$subcat_list->get_cat_name()?></td>
</tr>
<tr>
	...
	<td height="20" valign="top" class="value" align="left" colspan="2"><select name="subcat" style="width:300px;"><?php
			foreach($parent_list as $pobj)
			{
				...
				{
?><option value="<?=$pobj->get_id()?>" <?=($pobj->get_id() == $subcat_list->get_sub_cat_id()?"SELECTED":"")?>><?=$pobj->get_name()?></option><?php
				}
			}
	?></select></td>
</tr>
...
<tr>
	...
	<td height="20" valign="top" class="value" align="left" colspan="2"><div id='c<?=$cat_obj->get_id()?>' class="layer<?=$cat_obj->get_level()?>" style="margin-left:6px;"><script language="javascript">showNextLayer('<?=$cat_obj->get_id()?>','<?=$cat_obj->get_level()?>','c<?=$cat_obj->get_id()?>');</script></div>
	</td>
<tr>
</table>
<table border="0" cellpadding="0" cellspacing="0" height="40" class="page_header" width="100%">
<tr>
	<td align="left" style="padding-left:8px;" width="50%" style="padding-left:8px"><input type="button" name="back" value="Back To Main Page" onClick="Redirect('<?=base_url()."marketing/category/top/"?>')"></td>
	<td align="right" style="padding-right:8px" ><input type="submit" name="submit" id="submit" value="<?=$lang["update_category"]?>" style="font-size:11px"></td>
</tr>
</table>
...
{
?>
<iframe name="frame2" src="<?=base_url()."marketing/ra_prod_cat/view/?sscat=".$cat_obj->get_id()?>&locked=1" scrolling="no" width="100%" frameborder="0" onLoad="SetFrameHeight(this)"></iframe>
<iframe name="frame1" src="<?=base_url()."marketing/category/view_scpv/?subcat_id=".$cat_obj->get_id()?>" scrolling="no" width="100%" frameborder="0" onLoad="SetFrameHeight(this)"></iframe>
<iframe name="frame3" src="<?=base_url()."marketing/category/view_prod_spec/".$cat_obj->get_id()?>" scrolling="no" width="100%" frameborder="0" onLoad="SetFrameHeight(this)"></iframe>
<?php
}
?>
<iframe id="update_menu" src="" frameborder="0" name="left" id="left" marginwidth="0" marginheight="0" hspace=0 vspace=0 width="0"></iframe>
</form>
</div>
<script>
