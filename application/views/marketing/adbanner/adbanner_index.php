...
<div id="main">
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tr>
            ...
            <td width="400" align="right" class="title"><input type="button" name="banlist"
                    ...
                    style="font-size:11px;width:120px"
                    onclick="self.location.href='<?= base_url() . "marketing/adbanner/upload" ?>'"></td>
        </tr>
        <tr>
    ...
    <form name="bannerlist" action="<?= $_SERVER["PHP_SELF"] ?>" method="POST">
        <table cellspacing="1" cellpadding="4" border="0" bgcolor="#bbbbff" width="100%">
            ...
            foreach ($adbanner_list as $obj) {
                ?>
                <tr>
                    <td bgcolor="#ddddff" width="142"
                        valign="top"><?= ($obj->get_cat_id() != 0 ? $obj->get_name() : $lang["top_page"]) ?></td>
                            ...
                            <tr>
                                <td id="bcell<?= $obj->get_id() ?>" height="160" align="center">
                                    ...
                                    if ($obj->get_bannerfile() != "") {
                                        ...
                                        <img  src="<?= base_url() . "images/adbanner/" . $obj->get_bannerfile() ?>"
                                              border="0"><?= ($obj->get_bannerlink() != '' ? "</a>" : "") ?>
                                    <?php
                                    } else {
                                        echo "&nbsp;";
                                    }
    ...
    <span style="padding-right:15px;"><?= $lang["foreground"] ?>&nbsp;&nbsp;<select name="bimage<?= $obj->get_id() ?>"
                                                                                    onChange='changeCellImage("bcell<?= $obj->get_id() ?>","<?= $image_dir ?>",this.value)'
                                                                                    style="width:200px;">
            <option value="">-- <?= $lang["not_using"] ?> --</option><?php

            foreach ($option_list as $value) {
                if ($value != "temp") {
                    ?>
                    <option
                    value="<?= $value ?>" <?= ($obj->get_bannerfile() == $value ? "SELECTED" : "") ?>><?= $value ?></option><?php
                }
            }
            ?></select>&nbsp;&nbsp;<?= $lang["ilink"] ?></span><input name="link<?= $obj->get_id() ?>"
                                                                      value="<?= $obj->get_bannerlink() ?>"
