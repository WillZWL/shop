    ...
    <?php if ($showcontent) { ?>
        <form name="fm" method="get">
            <table border="0" cellpadding="0" cellspacing="1" bgcolor="#000000" width="100%" class="tb_pad">
                ...
                <tr class="header">
                    ...
                    <td><a href="#"
                           onClick="SortCol(document.fm, 'qty', '<?= $xsort["qty"] ?>')"><?= $lang["qty"] ?> <?= $sortimg["qty"] ?></a>
                    </td>
                    <td><a href="#"
                           onClick="SortCol(document.fm, 'supplier_name', '<?= $xsort["supplier_name"] ?>')"><?= $lang["supplier_name"] ?> <?= $sortimg["supplier_name"] ?></a>
                    </td>
                ...
                <tr class="search" id="tr_search" <?= $searchdisplay ?>>
                    ...
                    <td><input name="shipment_id" class="input"
                               value="<?= htmlspecialchars($this->input->get("ship_ref")) ?>"></td>
        ...
        <form action="<?= $_SERVER["PHP_SELF"] ?>" method="POST" name="confirmForm">
                    ...
                    if ($total > 0) {
                        foreach ($result as $inv_obj) {
                            ...
                            <tr bgcolor="<?= $cur_color ?>" onmouseover="ChgBg(this, '#DDFFDD')"
                                ...
                                <td><?= $inv_obj->get_shipment_id() ?><input name="shipment_id[<?= $i ?>]"
                                                                             id="shipment_id[<?= $i ?>]" type="hidden"
                                                                             value="<?= $inv_obj->get_shipment_id() ?>">
                                </td>
                                <td><?= $inv_obj->get_sku() ?></td>
                                <td><?= $inv_obj->get_prod_name() ?></td>
                                <td><?= $inv_obj->get_shipped_qty() ?><input name="sq[<?= $i ?>]" id="sq[<?= $i ?>]"
                                                                             type="hidden"
                                                                             value="<?= $inv_obj->get_shipped_qty() ?>">
                                </td>
                                <td><?= $inv_obj->get_supplier_name() ?></td>
                                <td><input class="input" type="text" name="rqty[<?= $i ?>]" id="rqty[<?= $i ?>]"
                                           value="<?= $inv_obj->get_shipped_qty() ?>" onKeyUp="checkQty('<?= $i ?>')">
                                </td>
                                <td><select class="input" name="reason[<?= $i ?>]" id="reason[<?= $i ?>]" DISABLED>
                                        <option value="OTH">Others</option>
                                    </select></td>
                                <td><input class="input" type="text" name="remarks[<?= $i ?>]" id="remarks[<?= $i ?>]"
                                           value=""></td>
                                <td>&nbsp;</td>
                            </tr>
                            <?php
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" class="tb_pad" bgcolor="#333333">
                <tr>
                    <td height="30" align="right"><input type="button" value="<?= $lang["confirm_quantity"] ?>"
