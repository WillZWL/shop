...
<div id="main">
    ...
    <form name="fm" action='<?= base_url() . "report/compensation_report/export_csv"; ?>' method="post"
    ...
    <form name="fm_2" action='<?= base_url() . "report/compensation_report/index"; ?>' method="post"
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%" class="tb_list" style="text-align:center">
        ...
        <form name="fm_3" method="get" onSubmit="return CheckForm(this)">
            <tr class="header">
                ...
                <td><a href="#"
                       onClick="SortCol(document.fm_3, 'so.platform_id', '<?= $xsort["so.platform_id"] ?>')"><?= $lang["platform_id"] ?> <?= $sortimg["so.platform_id"] ?></a>
                </td>
                <td><a href="#"
                       onClick="SortCol(document.fm_3, 'soc.item_sku', '<?= $xsort["soc.item_sku"] ?>')"><?= $lang["sku"] ?><?= $sortimg["soc.item_sku"] ?></a>
                </td>
                <td><a href="#"
                       onClick="SortCol(document.fm_3, 'p.name', '<?= $xsort["p.name"] ?>')"><?= $lang["product_name"] ?><?= $sortimg["p.name"] ?></a>
                </td>
                <td><a href="#"
                       onClick="SortCol(document.fm_3, 'soch.note', '<?= $xsort["soch.note"] ?>')"><?= $lang["reason"] ?><?= $sortimg["soch.note"] ?></a>
                </td>
                <td><a href="#"
                       onClick="SortCol(document.fm_3, 'soc.create_on', '<?= $xsort["soc.create_on"] ?>')"><?= $lang["request_date"] ?><?= $sortimg["soc.create_on"] ?></a>
                </td>
                <td><a href="#"
                       onClick="SortCol(document.fm_3, 'soc.create_by', '<?= $xsort["soc.create_by"] ?>')"><?= $lang["request_by"] ?><?= $sortimg["soc.create_by"] ?></a>
                </td>
                <td><a href="#"
                       onClick="SortCol(document.fm_3, 'soc.modify_on', '<?= $xsort["soc.modify_on"] ?>')"><?= $lang["approve_date"] ?><?= $sortimg["soc.modify_on"] ?></a>
                </td>
                <td><a href="#"
                       onClick="SortCol(document.fm_3, 'soc.modify_by', '<?= $xsort["soc.modify_by"] ?>')"><?= $lang["approve_by"] ?><?= $sortimg["soc.modify_by"] ?></a>
                </td>
                <td></td>
            </tr>
            <tr class="search" id="tr_search" <?= $searchdisplay ?>>
                ...
                <td><input name="product_name" type="text" class="input"
                           value="<?= $this->input->get("product_name") ?>"></td>
                <td><input name="reason" type="text" class="input" value="<?= $this->input->get("reason") ?>"></td>
                <td><input name="request_date" type="text" class="input"
                           value="<?= $this->input->get("request_date") ?>"></td>
                <td><input name="request_by" type="text" class="input" value="<?= $this->input->get("request_by") ?>">
                </td>
                <td><input name="approved_date" type="text" class="input"
                           value="<?= $this->input->get("approved_date") ?>"></td>
                <td><input name="approved_by" type="text" class="input" value="<?= $this->input->get("approved_by") ?>">
                </td>
