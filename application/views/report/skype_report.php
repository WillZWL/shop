...
<div id="main">
            ...
            <tr height="70">
                <td align="left" style="padding-left:8px;">
                    ...
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="line-height:8px;">
                        <tr>
                            <td><?= $lang['sku'] ?>:<select name="inclusion[sku]"
                                                            onchange="if(this.value=='all'){document.getElementById('sku').disabled=true;document.getElementById('sku').value='';}else{document.getElementById('sku').disabled = false;}">
                                    <option value="all">All</option>
                                    <option value="1">Include</option>
                                    <option value="0">Exclude</option>
                                </select><input type="text" id="sku" name="sku" disabled></input></td>
                            <td><?= $lang['conv_site_id'] ?>:<select name="inclusion[conv_site_id]"
                                                                     onchange="if(this.value=='all'){document.getElementById('conv_site_id').disabled=true;document.getElementById('conv_site_id').value='';}else{document.getElementById('conv_site_id').disabled = false;}">
                                    <option value="all">All</option>
                                    <option value="1">Include</option>
                                    <option value="0">Exclude</option>
                                </select><input type="text" id="conv_site_id" name="conv_site_id" disabled></input></td>
                            <td><?= $lang['promotion_code'] ?>:<select name="inclusion[promotion_code]"
                                                                       onchange="if(this.value=='all'){document.getElementById('promotion_code').disabled=true;document.getElementById('promotion_code').value='';}else{document.getElementById('promotion_code').disabled = false;}">
                                    <option value="all">All</option>
                                    <option value="1">Include</option>
                                    <option value="0">Exclude</option>
                                </select><input type="text" id="promotion_code" name="promotion_code" disabled></input>
                            </td>
                        </tr>
                    </table>
                </td>
    ...
    <iframe name="report" id="report" src="<?= base_url() ?>report/skype_report/query" width="1259"
