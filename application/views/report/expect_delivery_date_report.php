...
<form name="fm" action='<?=base_url()."report/expect_delivery_date_report/export_csv";?>' method="post" onSubmit="return verfiy_date(this)">
...
<form name="fm_2" action='<?=base_url()."report/expect_delivery_date_report/index";?>' method="post" onSubmit="return getDateInfo(this)">
<tr>
...
<form name="fm_3" method="get" onSubmit="return CheckForm(this)">
	<tr class="header">
		...
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.platfrom_id', '<?=$xsort["so.platfrom_id"]?>')"><?=$lang["platform"]?><?=$sortimg["so.platfrom_id"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'sops.payment_gateway_id', '<?=$xsort["sops.payment_gateway_id"]?>')"><?=$lang["paym_gateway"]?><?=$sortimg["sops.payment_gateway_id"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.platform_id', '<?=$xsort["so.platform_id"]?>')"><?=$lang["platform_order_id"]?><?=$sortimg["so.platform_id"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'c.ext_client_id', '<?=$xsort["c.ext_client_id"]?>')"><?=$lang["ebay_id"]?><?=$sortimg["c.ext_client_id"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.txn_id', '<?=$xsort["so.txn_id"]?>')"><?=$lang["transaction_number"]?><?=$sortimg["so.txn_id"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.amount', '<?=$xsort["so.amount"]?>')"><?=$lang["order_amout"]?><?=$sortimg["so.amount"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.order_create_date', '<?=$xsort["so.order_create_date"]?>')"><?=$lang["create_date"]?><?=$sortimg["so.order_create_date"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.expect_delivery_date', '<?=$xsort["so.expect_delivery_date"]?>')"><?=$lang["edd"]?><?=$sortimg["so.expect_delivery_date"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.bill_name', '<?=$xsort["so.bill_name"]?>')"><?=$lang["client_name"]?><?=$sortimg["so.bill_name"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'c.email', '<?=$xsort["c.email"]?>')"><?=$lang["client_email"]?><?=$sortimg["c.email"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'contact_no', '<?=$xsort["contact_no"]?>')"><?=$lang["contact_number"]?><?=$sortimg["contact_no"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.dispatch_date', '<?=$xsort["so.dispatch_date"]?>')"><?=$lang["shipped_on"]?><?=$sortimg["so.dispatch_date"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.status', '<?=$xsort["so.status"]?>')"><?=$lang["order_status"]?><?=$sortimg["so.status"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.hold_status', '<?=$xsort["so.hold_status"]?>')"><?=$lang["hold_status"]?><?=$sortimg["so.hold_status"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.refund_status', '<?=$xsort["so.refund_status"]?>')"><?=$lang["refund_status"]?><?=$sortimg["so.refund_status"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'sps.score', '<?=$xsort["sps.score"]?>')"><?=$lang["priority_score"]?><?=$sortimg["sps.score"]?></a></td>
		<td><a href="#" onClick="SortCol(document.fm_3, 'so.modify_by', '<?=$xsort["so.modify_by"]?>')"><?=$lang["modify_by"]?><?=$sortimg["so.modify_by"]?></a></td>
		<td></td>
	</tr>
	<tr class="search" id="tr_search" <?=$searchdisplay?>>
		...
		<td><input name="platform_gateway_id" type="text" class="input" value="<?=$this->input->get("platform_gateway_id")?>"></td>
		<td><input name="platform_order_id" type="text" class="input" value="<?=$this->input->get("platform_order_id")?>"></td>
		<td><input name="ext_client_id" type="text" class="input" value="<?=$this->input->get("ext_client_id")?>"></td>
		...
		<td><input name="order_create_date" type="text" class="input" value="<?=$this->input->get("order_create_date")?>"></td>
		<td><input name="expect_delivery_date" type="text" class="input" value="<?=$this->input->get("expect_delivery_date")?>"></td>
		<td><input name="bill_name" type="text" class="input" value="<?=$this->input->get("bill_name")?>"></td>
		<td><input name="email" type="text" class="input" value="<?=$this->input->get("email")?>"></td>
		<td><input name="contact_no" type="text" class="input" value="<?=$this->input->get("contact_no")?>"></td>
		<td><input name="dispatch_date" type="text" class="input" value="<?=$this->input->get("dispatch_date")?>"></td>
		<td><input name="status" type="text" class="input" value="<?=$this->input->get("status")?>"></td>
		<td><input name="hlod_status" type="text" class="input" value="<?=$this->input->get("hlod_status")?>"></td>
		<td><input name="refund_status" type="text" class="input" value="<?=$this->input->get("refund_status")?>"></td>
		<td><input name="score" type="text" class="input" value="<?=$this->input->get("score")?>"></td>
		<td><input name="modify_by" type="text" class="input" value="<?=$this->input->get("modify_by")?>"></td>
