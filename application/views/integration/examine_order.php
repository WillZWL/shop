    ...
    <table border="0" cellpadding="0" cellspacing="1" width="100%" bgcolor="#cccccc" class="tb_list">
        ...
        foreach ($obj_list as $obj) {
            if ($obj->get_line_no() == $this->input->get("line_no")) {
                ...
                <tr class="row<?= $i++ ?>">
                    <form name="fm"
                          action="<?= base_url() . 'integration/integration/examine_order/' . $batch_id . '/' . $trans_id . '/' ?>"
                        ...
                        <td><input type="button" value="<?= $lang["submit_change"] ?>"
                                   onClick="if(CheckForm(this.form)) document.fm.submit();"></td>
