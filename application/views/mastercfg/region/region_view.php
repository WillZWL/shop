...
<tr>
    ...
    <td height="20" align="left" class="value">&nbsp;&nbsp;<input type="text" name="region_name" value="<?=$region_obj->get_region_name()?>" <?=(!$editable?"readonly":"")?> notEmpty></td>
</tr>
<tr>
    ...
    <td height="20" align="left" class="value">&nbsp;&nbsp;
        <select name="region_type" style="width:200px;" <?=($editable?"":"DISABLED")?>>
            <option value="C" <?=($region_obj->get_type() == "C"?"SELECTED":"")?>><?=$lang["courier"]?></option>
            <option value="S" <?=($region_obj->get_type() == "S"?"SELECTED":"")?>><?=$lang["sourcing"]?></option>
        </select>
    </td>
</tr>
<tr>
    <td height="20" width="250" class="field" align="right">&nbsp;&nbsp;<?=$lang["country_in_region"]?></td>
    <td height="20" aalign="left" class="value" style="padding:6px;">
        <table border="0" cellpadding="2" cellspacing="1" class="tb_list">
        <tr class="header">
            <?php if($editable) { ?>
            <th ><font color="#ffffff"><?=(!$editable?"&nbsp;":$lang["not_in_region"])?></font></th>
            <th >&nbsp;</th>
            <?php } ?>
...
?></select></td>
        ...
        <?}?>
        <td><select name="country[]" id='right' style='width:150px; height:300px;' multiple='multiple' <?=!$editable?"disabled":""?>><?php
...
<td align="right" style="padding-right:8px"><input type="button" value="<?=$lang["update_region"]?>" onclick="SelectAllItems(document.editform.elements['country[]']); if(CheckForm(this.form)) this.form.submit();" style="font-size:11px"></td>
</tr>
</table>
