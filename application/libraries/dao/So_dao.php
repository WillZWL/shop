...
//          $total_quantity += $rs[$i]->get_item_quantity();
    ...
    public function get_orders_for_dm($where = array(), $option = array(), $classname = "So_w_client_and_item_dto")
    {
        ...
        $this->db->where($where);
        $this->db->select("so.so_no, so.currency_id, so.amount, so.create_at, so.lang_id, so.fingerprintId, sps.payment_gateway_id, sps.risk_ref_3, sps.risk_ref_4, sps.payer_email,
