...
class Batch_service extends Base_service {
	...
	function cron_ixtens_reprice($platform = "AMUK")
	{
		...
		$ftp = $this->get_ftp();
		$ftp_obj = $this->get_fi_dao()->get(array("name"=>"IXTENS_".$platform));
	...
	public function generate_sli_prod_feed($output = "file", $where=array())
	{
		...
		else
		{
			if($output == "xml")
			{
				...
				$output = str_replace("<entity>","<product_list>",$output);
				$output = str_replace("</entity>","</product_list>",$output);
				$filepath = $this->get_config()->value_of("sli_xml_path");
			...
			else
			{
				...
				$encrypt = new CI_Encrypt();
				$ftp = $this->get_ftp();
				$ftp_obj = $this->get_fi_dao()->get(array("name"=>"SLI"));
...
//		echo "$platform";
	...
	public function proceed_amazon_batch($batch_id, $platform = "AMUK")
	{
		...
		$func_curr = $this->get_config()->value_of("func_curr_id");
			...
			foreach($order_list as $iso_obj)
			{
				...
				if($canupdate)
				{
					...
					else
					{
						...
						//$event->fire_event($func, $dto);
						$bobj = $this->get_dao()->get(array("id"=>$batch_id));
						$bobj->set_status("CE");
						$bobj->set_end_time(date("Y-m-d H:i:s"));
						$this->get_dao()->update($bobj);
...
//		$option["inventory"] = TRUE;
		...
		if ($prod_list = $price_srv->get_product_overview($where, $option))
		{
			foreach ($feeders as $feeder)
			{
					...
					if ($rs_file = $out_file[$feeder])
					{
						if ($fp = fopen($path."/".$rs_file, 'w'))
						{
							...
							foreach ($prod_list as $obj)
							{
								...
								$new_obj->set_url($this->get_config()->value_of("default_url")."/".str_replace(' ', '-', parse_url_char($new_obj->get_prod_name()))."/mainproduct/view/".$new_obj->get_sku()."/".$wysiwyg[$feeder]);
								...
								{
									$new_obj->set_image($this->get_config()->value_of("default_url")."/images/product/".$new_obj->get_sku().".".$new_obj->get_image());
								}
				...
				if ($ftp_obj = $this->get_fi_dao()->get(array("name"=>$feeder)))
				{
					$ftp->set_remote_site($server = $ftp_obj->get_server());
					$ftp->set_username($ftp_obj->get_username());
					$ftp->set_password($encrypt->decode($ftp_obj->get_password()));
...
//		if($feeder =='REEVOO'){ $proddesc =$EMAXproddesc; }
		if($feeder =='REEVOO'){ $proddesc =$RVproddesc; }
	...
	public function create_system_at_job($run_minute_after, $command)
	{
	...
	public function schedule_php_process($run_minute_after, $php_file)
	{
		$command = 'cd ' . APPPATH . '../admincentre/; ';
		$command .= '/usr/bin/php index.php ' . $php_file;

		$this->create_system_at_job($run_minute_after, $command);
	}
}
