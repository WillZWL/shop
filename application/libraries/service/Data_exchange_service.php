...
interface in_converter
{
    public function in_convert();
}

interface out_converter
{
    public function out_convert();
}

class Data_exchange_service extends Base_service
{
    ...
    public function convert(in_converter $import, out_converter $export = NULL)
    {
        $this->import = $import;
        if (empty($export)) {
            return $this->import->in_convert();
        } else {
            $this->export = $export;
            $this->export->set_input($this->import->in_convert());
            return $this->export->out_convert();
        }
    }
...
class Csv_to_xml implements in_converter
{
    ...
    function Csv_to_xml($input = "", $map_file = "", $first_line_heading = TRUE, $delimiter = ",", $checkQuote = TRUE)
    {
        $CI =& get_instance();
        $this->load = $CI->load;
...
// XML to XML
class Xml_to_xml implements in_converter, out_converter
{
    ...
    function Xml_to_xml($input = "", $map_file = "", $basetag = "entity")
    {
        $CI =& get_instance();
        $this->load = $CI->load;
...
// VO to XML
class Vo_to_xml implements in_converter
{

    private $input;
    private $map_file;

    function Vo_to_xml($input = "", $map_file = "")
    {
        $CI =& get_instance();
        $this->load = $CI->load;
    ...
    public function in_convert($need_cdata = true)
    {
        ...
        if ($result) {
            foreach ($result as $rsdata) {
                if ($container == "") {
                    ...
                    if (substr($rscontainer, -3) == "_vo") {
                        $rscontainer = substr($rscontainer, 0, -3);
                    }
                    //added by Jack
                    if (substr($rscontainer, -4) == "_dto") {
                        $rscontainer = substr($rscontainer, 0, -4);
                    }
                    $container = $rscontainer;
                }
                $output .= "\t<{$container}>\n";
                if ($this->map_file != "") {
                    foreach ($mapping as $rskey => $rsvalue) {
                        ...
                        $data = @call_user_func(array($rsdata, "get_" . $p_key));
                        if ($sub_key) {
...
// XML to CSV
class Xml_to_csv implements out_converter
{
    ...
    function Xml_to_csv($input = "", $map_file = "", $first_line_heading = TRUE, $delimiter = ",")
    {
        $CI =& get_instance();
        $this->load = $CI->load;
...
// XML to XLS
class Xml_to_xls implements out_converter
{
    private $input;
    private $map_file;

    function Xml_to_xls($input = "", $map_file = "", $first_line_heading = TRUE, $sheet_map_file = "", $display_sheet_key = TRUE)
    {
        $CI =& get_instance();
        $this->load = $CI->load;
...
// XML to VO
class Xml_to_vo implements out_converter
{

    private $input;
    private $map_file;

    function Xml_to_vo($input = "", $map_file = "")
    {
        $CI =& get_instance();
        $this->load = $CI->load;
    ...
    public function out_convert()
    {
        ...
        foreach ($result as $rscontainer => $row) {
            ...
            if (substr($container, -4) == "_dto") {
                $vo_file = APPPATH . "/libraries/dto/" . strtolower(substr($container, 0, -4)) . "_dto.php";
                $vo_class = ucwords($container);
            } elseif (substr($container, -3) == "_vo") {
                $vo_file = APPPATH . "/libraries/vo/" . strtolower(substr($container, 0, -3)) . "_vo.php";
                $vo_class = ucwords($container);
            } else {
                $vo_file = APPPATH . "/libraries/vo/" . strtolower($container) . "_vo.php";
                $vo_class = ucwords($container) . "_vo";
            }
                    ...
                    foreach ($mapping as $rskey => $rsvalue) {
                        if (@isset($row->$rskey)) {
                            ...
                            foreach ($tmp as $rsval) {
                                if (substr($rsval, 0, 1) != "+") {
                                    @call_user_func(array($vo, "set_" . $rsval), (string)$row->$rskey);
                                } else {
                                    $field = substr($rsval, 1);
                                    $content = @call_user_func(array($vo, "get_" . $field));
                                    if (trim((string)$row->$rskey) != "") {
                                        @call_user_func(array($vo, "set_" . $field), $content . ", " . (string)$row->$rskey);
                                    }
                                }
                            }
                        }
