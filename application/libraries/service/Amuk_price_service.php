...
class Amuk_price_service extends Platform_price_service
{
...
//      $this->get_dto()->set_admin_fee(0);
        ...
        if ($this->get_dto()->get_shiptype() == 2)
        {
            $this->get_dto()->set_freight_cost(0);
        }
    ...
    private function calc_vat($src = NULL)
    {
        ...
        {
            $vat = ($dto->get_declared_value() + $dto->get_admin_fee())* $dto->get_vat_percent()/100;
    ...
    public function calc_profit($src = NULL)
    {
        ...
        $dto->set_cost(
            ...
            + round($dto->get_freight_cost()*$this->adj_rate, 2)
            + round($dto->get_delivery_cost(), 2)
            + round($dto->get_supplier_cost()*$this->adj_rate, 2)
    ...
    public function draw_table_row_for_pricing_tool($default_shiptype="")
    {
        ...
        $this->get_dto()->set_freight_cost(round($this->get_dto()->get_freight_cost() * $this->adj_rate, 2));
        $this->get_dto()->set_supplier_cost(round($this->get_dto()->get_supplier_cost() * $this->adj_rate, 2));
        ...
        $table_row .='<tr id="row['.$type.']" style="background-color:'.$bgcolor.'">
                        ...
                        <td><input type="hidden" id="declared_rate['.$type.']" value="'.$this->get_dto()->get_declared_pcent().'">
                            ...
                            <input type="hidden" id="scost['.$type.']" value="'.round($this->get_dto()->get_supplier_cost(),2).'">
                        </td>
                     </tr>
                    '."\n";
        return $table_row;
    ...
    public function get_product_overview_tr($where=array(), $option=array(), $classname="", $lang=array())
    {
        ...
        {
            $this->price_margin_service->refresh_latest_margin(array("v_prod_overview_w_update_time.platform_id"=>"AMUK"));
        }
        if ($objlist = $this->product_service->get_dao()->get_product_overview($where, $option, $classname))
        {
            ...
            $ar_ap_status = array(""=>"", "Y" => $lang["enabled"], "N" => $lang["disabled"]);
            ...
            foreach ($objlist as $obj)
            {
                ...
                foreach ($ar_ap_status as $rkey=>$rvalue)
                {
                    $ap_status .= "<option value='{$rkey}' {$selected_ap[$rkey]}>{$rvalue}</option>";
                }
                $sku = $obj->get_sku();
                ...
                $weblink = ($asin==""?"#":"http://www.amazon.co.uk/dp/".$asin);
                ...
                $data["tr"] .= "
                                <tr class='row{$cur_row}' onMouseOver=\"AddClassName(this, 'highlight')\" onMouseOut=\"RemoveClassName(this, 'highlight')\">
                                    <td></td>
                                    <td><a href='".base_url()."marketing/pricing_tool_amuk/view/{$sku}?target=overview' target='amukpt' onClick=\"Pop('".base_url()."marketing/pricing_tool_amuk/view/{$sku}?target=overview','amukpt');\">{$sku}</a></td>
                                    ...
                                    <td><input class='input' name='price[{$sku}][latency]' value='{$this->get_dto()->get_latency()}' onChange='needToConfirm=true'></td>
                                    <td>
                                        <select name='price[{$sku}][auto_price]' onChange='needToConfirm=true'>
                                                {$ap_status}
                                        </select>
                                    </td>
                ...
                $this->get_dto()->set_freight_cost(round($this->get_dto()->get_freight_cost() * $this->adj_rate, 2));
                $this->get_dto()->set_supplier_cost(round($this->get_dto()->get_supplier_cost() * $this->adj_rate, 2));
                $data["js"] .= "prod['{$sku}'] = {'shiptype':{$this->get_dto()->get_shiptype()},'clearance':{$this->get_dto()->get_clearance()},'declared_rate':{$this->get_dto()->get_declared_pcent()},'payment_charge_rate':{$this->get_dto()->get_payment_charge_percent()},'vat_percent':{$this->get_dto()->get_vat_percent()},'duty_percent':{$this->get_dto()->get_duty_pcent()},'free_delivery_limit':{$this->get_dto()->get_free_delivery_limit()},'default_delivery_charge':{$this->get_dto()->get_default_delivery_charge()},'admin_fee':{$this->get_dto()->get_admin_fee()},'freight_cost':{$this->get_dto()->get_freight_cost()},'delivery_cost':{$this->get_dto()->get_delivery_cost()},'supplier_cost':{$this->get_dto()->get_supplier_cost()},'platform_delivery_charge':{$this->get_dto()->get_platform_delivery_charge()},'platform_commission':{$this->get_dto()->get_platform_commission()}};
                                ";
                $i++;
            }
            if ($i>0)
            {
                $data["tr"] .= "<tr class='tb_brow' align='right'><td colspan='19'>{$lang['currency']}: <b>{$obj->get_platform_currency_id()}<b></td></tr>";
                $data["js"] .= "</script>";
            }
            else
            {
    ...
    public function get_ixtens_reprice_list($where=array(), $option=array(), $classname="Product_cost_dto")
    {
        ...
        if ($objlist = $this->product_service->get_dao()->get_ixtens_repice_list($where, $option, $classname))
        {
            ...
            foreach ($objlist as $obj)
            {
                ...
                $obj->set_price($obj->get_int_price());
                $this->set_dto($obj);
                $this->calc_profit();
                $data["margin"][$sku] = $obj->get_margin();
            }
            return $data;
        }
        return FALSE;
    ...
    public function get_min_price($margin, $dto = NULL)
    {
        ...
        return number_format(
            ...
            + 100*round($fc, 2)*$this->adj_rate
            + 100*round($dto->get_delivery_cost(), 2)
            + 100*round($dto->get_supplier_cost(), 2)*$this->adj_rate
