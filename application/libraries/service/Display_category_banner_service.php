...
class Display_category_banner_service extends Base_service
{
    ...
    {
        parent::__construct();
        include_once(APPPATH . "libraries/dao/Display_category_banner_dao.php");
        $this->set_dao(new Display_category_banner_dao());
    ...
    public function get_list_with_name($level, $parent)
    {
        return $this->get_dao()->get_list_with_name($level, $parent);
    }
    ...
    {
        return $this->get_dao()->get_db_w_graphic($catid, $banner_type, $display_id, $position_id, $slide_id, $country_id, $lang_id, $usage, $backup_image);
    }
    ...
    {
        return $this->get_dbc_dao()->get_different_country_list($catid, $display_id, $lang_id);
    }
    ...
    public function get_publish_banner($catid, $display_id, $position_id, $country_id, $lang_id, $usage = "PB")
    {
        ...
        if ($dbc) {
            ...
            if ($banner["banner_type"] == "I") {
                $db_obj = $this->get_dao()->get_db_w_graphic($catid, $banner["banner_type"], $display_id, $position_id, 0, $country_id, $lang_id, $usage, FALSE);
                if ($db_obj) {
                ...
                $db_obj = $this->get_dao()->get_db_w_graphic($catid, $banner["banner_type"], $display_id, $position_id, 0, $country_id, $lang_id, $usage);
                if ($db_obj->get_status() == 1 && file_exists(GRAPHIC_PH . $db_obj->get_graphic_location() . $db_obj->get_graphic_file())) {
                    $redirect_link[0] = $db_obj->get_link();
                    $link_type[0] = $db_obj->get_link_type();
                ...
                $image_db_obj = $this->get_dao()->get_db_w_graphic($catid, $banner["banner_type"], $display_id, $position_id, 0, $country_id, $lang_id, $usage, TRUE);
                ...
                if ($country_id) {
                    $db_list = $this->get_dao()->get_list(array('catid' => $catid, 'position_id' => $position_id, 'display_id' => $display_id, 'usage' => $usage, 'country_id' => $country_id, 'lang_id' => $lang_id, 'status' => 1), array("orderby" => "priority DESC"));
                } else {
                    $db_list = $this->get_dao()->get_list(array('catid' => $catid, 'position_id' => $position_id, 'display_id' => $display_id, 'usage' => $usage, 'country_id IS NULL' => NULL, 'lang_id' => $lang_id, 'status' => 1), array("orderby" => "priority DESC"));
                }
                if ($db_list) {
                    $num = 0;
                    foreach ($db_list AS $db_temp) {
                        $db_obj = $this->get_dao()->get_db_w_graphic($catid, $banner["banner_type"], $display_id, $position_id, $db_temp->get_slide_id(), $country_id, $lang_id, $usage);
