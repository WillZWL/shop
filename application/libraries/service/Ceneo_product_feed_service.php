...
class Ceneo_product_feed_service extends Data_feed_service
{
    protected $id = "Ceneo Product Feed";
    private $prod_srv;
    private $price_srv;
    private $cat_srv;
    private $now_date;
    ...
    {
        // ceneo will retrieve file from valuebasket.com/affiliates/external_query/get_ceneo_xml/WEBPL
        define('DATAPATH', $this->get_config_srv()->value_of("data_path"));
        $filename = strtolower('valuebasket_ceneo_' . "$platform_id" . '.xml');
        $fp = fopen(DATAPATH . "feeds/ceneo/ftp/$platform_id/" . $filename, 'r');

        $content = file_get_contents(DATAPATH . "feeds/ceneo/ftp/$platform_id/" . $filename);
