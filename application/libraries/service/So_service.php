...
class So_service extends Base_service
{
    ...
    private $sor_sao;
    private $rma_dao;
    private $rma_history_dao;
    private $rma_notes_dao;
    private $socc_dao;
    private $son_dao;
    private $cart_srv;
    private $pbv_srv;
    ...
    private $event_srv;
    private $dex_service;
    private $crcy_service;
    private $price_service;
    private $domain_platform_service;
    private $wow_email_service;
    private $courier_service;
    private $payment_gateway_dao;
    private $sub_domain_srv;
    private $entity_srv;
    private $email_referral_list_service;
    private $fraudulent_order_service;
    private $delay_order_service;
    private $ca_service;
    private $dt_service;
    ...
    <center><table width="100%" cellspacing="0" id="checkout"><tr class="header"><td>Product</td><td>Qty</td><td>Total</td></tr>';
        ...
        #sbf #3746, only add complementary accessory for website orders
        // $get_ca = false;
        // if (substr(strtoupper($vars["platform_id"]), 0, 3) == "WEB")
        // {
        //  $get_ca = true;
        ...
        foreach ($cart_list["cart"] as $line_no => $soi) {
            ...
            //if ($vars["biz_type"]=="special")
            //{
            //  $soi_obj->set_unit_price(round($vars["soi_price"][$soi["sku"]], 2));
            //  $soi_obj->set_amount($vars["soi_price"][$soi["sku"]] * $soi["qty"]);
            //  $amount += $vars["soi_price"][$soi["sku"]];
            //}
            //else
            //{
...
//              $_SESSION["NOTICE"] = "Error: ".__LINE__." ".$dao->db->_error_message()." -- ".$dao->db->last_query();
    ...
    public function split_order_to_so($so_no, $order_group)
    {
        ...
        $ordernotes_dao = $this->get_son_dao();
        $so_priority_score_dao = $this->get_so_ps_srv()->get_dao();
        $so_holdreason_dao = $this->get_sohr_dao();
        ...
        if ($so_obj = $so_dao->get(array("so_no" => $so_no))) {
            ...
            // allow split for SPECIAL ORDERS (no payment gateway), so commented out the following
            // if(! ($so_payment_status_obj = $so_payment_status_dao->get(array("so_no"=>$so_no))) )
            // {
            //  $ret["status"] = FALSE;
            //  $ret["message"] = __LINE__." so_service. Error: so_payment_status_obj not found for $so_no.";
            //  return $ret;
            // }
...
html;
                    ...
                    // $replace["platform_id"] = $cur_platform_id;
                    ...
                    foreach ($itemlist as $item_obj) {
                        ...
                        $item_information .= '<tr>
                                                ...
                                                <td align="right">' . platform_curr_format($item_obj->get_unit_price()) . '</td>
                    ...
                    if ($extobj) {
                        if ($extobj->get_offline_fee() > 0) {
                            $replace["offline_fee"] = '<tr>
                                ...
                                <td align="right" bgcolor="#F0F0F0" valign="top" style="border:1px solid #BBBBBB; border-width:0px 1px 1px 1px;"><b>' . platform_curr_format($extobj->get_offline_fee()) . '</b></td>
                            </tr>';
                    ...
                    $replace["grand_total"] = platform_curr_format($so_obj->get_amount());
...
//EDD is earlier, alert
                ...
                if (!$so_obj) {
                    ...
                    foreach ($itemlist as $item_obj) {
                        ...
                        if ($currency) {
                            ...
                            $declared_value_converted = $this->convert_currency($original_currency, $currency, $declared_value_converted);
                            $delivery_charge_converted = $this->convert_currency($original_currency, $currency, $delivery_charge_converted);
                            $processing_fee_converted = $this->convert_currency($original_currency, $currency, $processing_fee_converted);
                        } else {
                            $currency = "USD";
                        }
    ...
    public function get_credit_check_list($where = array(), $option = array(), $type = "")
    {
        ...
        if ($obj_list = $this->get_dao()->get_credit_check_list($where, $option, $type)) {
            foreach ($obj_list as $obj) {
                ...
                $obj->set_items($item);
                $sor_obj = $this->get_sor_dao()->get(array("so_no" => $obj->get_so_no()));
    ...
    public function get_next_sh_no($so_no)
    {
...
//                              $row->set_actual_cost(0.00);
                ...
                case "ARAMEX_COD":
                    ...
                    // echo "<pre>"; var_dump($data_out); die();
                    $out_xml = new Vo_to_xml($data_out, '');
...
#       $replace["delivery_address_text"] = str_replace("||", "|", $so_obj->get_delivery_address())
        ...
        $replace["amount"] = platform_curr_format($so_obj->get_amount(), 0);
...
//              $email_sender = 'no-reply@valuebasket.com';
        ...
        foreach ($so_items as $item) {
            ...
            $replace["so_items_text"] .=
                $item->get_name() . $item_name_tab . $cur_qty . "\t" . platform_curr_format($price, 0) . "\t" . platform_curr_format($cur_sub_total, 0) . "\r\n";
            $replace["so_items"] .=
                "<tr>
...
//              $email_sender = 'no-reply@valuebasket.com';
    ...
    public function set_rr_dao(Base_dao $dao)
    {
        $this->rr_dao = $dao;
    }
    ...
    {
        return $this->get_prod_srv()->is_trial_software($sku);
    }

    public function is_software($sku = "")
    {
        return $this->get_prod_srv()->is_software($sku);
    }

    public function get_product_type_w_sku($sku = "")
    {
        return $this->get_prod_srv()->get_product_type_w_sku($sku);
    }

    public function get_reevoo_customer_feed_dto($last_access_time = "")
    {
        return $this->get_dao()->get_reevoo_customer_feed_dto($last_access_time);
    }

    public function insert_sops($obj)
    {
        return $this->get_sops_dao()->insert($obj);
    }

    public function update_sops($obj)
    {
        return $this->get_sops_dao()->update($obj);
    }
    ...
    {
        return $this->get_rma_notes_dao()->insert($obj);
    }
    ...
    public function get_ebay_pending_shipment_update_orders($where = array(), $option = array())
    {
        return $this->get_dao()->get_ebay_pending_shipment_update_orders($where, $option);
    }
    ...
    {
        return $this->get_dao()->get_automated_feedback_email_content($where, $option);
    }
    ...
    public function get_priority_score($so_no, $result = null)
    {
        ...
        $score_christmas = $this->get_priority_score_christmas($so_no, $result);

        if ($score_christmas > 0)
            return $score_christmas;
        else
            return $this->get_priority_score_base($so_no, $result);
    }

    public function get_priority_score_christmas($so_no, $result = null)
    {
        $score = 0;
        $year = date("Y");
        $month = date("n");
...
//OFFLINE BULK SALES
                    ...
                    if ((($soex_obj = $this->get_soext_dao()->get(array("so_no"=>$so_no))) !== FALSE) && ($soex_obj))
                    {
                        if (($soex_obj->get_order_reason() == "APS Color Change")
                            || ($soex_obj->get_order_reason() == "APS Cheaper")
                            || ($soex_obj->get_order_reason() == "APS Payment Required")
...
*/
    ...
    {
        return $this->get_sosh_dao()->get_shipping_info($where);
    }

    public function get_lifetime_transaction_by_client_id($client_id)
    {
        return $this->get_dao()->get_lifetime_transaction_by_client_id($client_id);
    }
    ...
    {
        return $this->get_dao()->get_distinct_client_id_list($where, $option);
    }

    public function get_rma_customer_email_address($past_day)
    {
        return $this->get_dao()->get_rma_customer_email_address($past_day);
    }

    public function get_aftership_data($where = array(), $option = array())
    {
        return $this->get_dao()->get_aftership_data($where, $option);
    }

    public function get_aftership_report_for_ftp($where = array(), $option = array())
    {
...
//                                  error_log('wowo4');
                                    ...
                                    $this->get_socc_dao()->$action($socc_obj);

                                    $so_obj->set_status(0);
                                    $so_obj->set_hold_status(0);
                                    $this->get_dao()->update($so_obj);
                                    $this->get_socc_dao()->trans_complete();

                                    //add an order note
                                    $order_note_vo = $this->get_son_dao()->get();
                                    $order_note_vo->set_so_no($so_no);
                                    $order_note_vo->set_note("system inactivate, blacklisted client");
                                    $this->get_son_dao()->insert($order_note_vo);
    ...
    {
        $this->get_dao()->cancel_order($age_in_days);
    }
