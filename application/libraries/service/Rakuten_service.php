...
class Rakuten_service
{
	...
	public function get_orders_list($country_id = "ES", $orderno="", $test = FALSE)
	{
		...
		if(isset($_GET["special"]))
		{
			...
			$this->special_report(); die();
		}
		if(isset($_GET["importspecial"]))
		{
			// for own use, go to special_report() and input your list of desired orders
			$this->importspecial($orderno); die();
		}
		...
		if($orderno)
		{
			...
			$csv_data = $csvheader.$csv;
			header ("Content-Type: text/csv"); header("Content-Disposition: attachment; filename=singleorderinfo.csv"); echo $csv_data; die();
		}
		else
		{
			...
			switch ($getstatus)
			{
				case 'unfixed': 			$orderstatus = 'Unfixed'; break;
				case 'awaitingpayment': 	$orderstatus = 'AwaitingPayment'; break;
				case 'processingpayment': 	$orderstatus = 'ProcessingPayment'; break;
				case 'notshipped': 			$orderstatus = 'NotShipped'  ; break;
				case 'awaitingcompletion': 	$orderstatus = 'AwaitingCompletion' ; break;
				case 'complete': 			$orderstatus = 'Complete'  ; break;
				case 'cancelled': 			$orderstatus = 'Cancelled'; break;
				case 'cancelledandrefund': 	$orderstatus = 'CancelledAndRefund'; break;
	...
	private function special_report()
	{
		...
		header ("Content-Type: text/csv"); header("Content-Disposition: attachment; filename=specialreport.csv"); echo $csv; die();
	}
