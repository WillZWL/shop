...
class Cart_session_service extends Base_service
{
    ...
    public function add($sku, $qty, $platform = null)
    {
        ...
        if ($qty <= 0) {
            ...
            foreach ($prod_list as $prod) {
                ...
                if (!($status == '2' && $listing_status == 'L' && $ws_qty > 0 && $price > 0)) {
                    $this->remove($sku, $platform);
                    $success = 0;
                    break;
    ...
    {
        $_SESSION["cart"][$platform][$sku] = $this->cart_format($qty, $website_status, $expect_delivery_date, $warranty_in_month);
    }

    public function cart_format($qty, $website_status = null, $expect_delivery_date, $warranty_in_month)
    {
        return [
            'qty' => $qty,
