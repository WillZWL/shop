...
class VbDataTransferProductContentExtendService extends VbDataTransferService
{
	...
	{
		parent::__construct();
		$this->setDao(new ProductContentExtendDao);
		...
		foreach($xml_vb->product as $pc)
		{
			...
			try
			{
				if ($fail_reason == "")
				{
					...
					$new_pc_obj["feature"] = $this->replaceSpecialChars($pc->feature);
					$new_pc_obj["feature_original"] = $this->replaceSpecialChars($pc->feature_original);
					$new_pc_obj["specification"] = $pc->specification;
					$new_pc_obj["spec_original"]  = $pc->spec_original;
					$new_pc_obj["requirement"] = $this->replaceSpecialChars($pc->requirement);
					$new_pc_obj["instruction"] = $this->replaceSpecialChars($pc->instruction);
				...
				elseif ($sku != "" && $sku != null)
				{
					...
					$new_pc_obj->setFeature($this->replaceSpecialChars($pc->feature));
					$new_pc_obj->setFeatureOriginal($this->replaceSpecialChars($pc->feature_original));
					$new_pc_obj->setSpecification($pc->specification);
					$new_pc_obj->setSpecOriginal($pc->spec_original);
					$new_pc_obj->setRequirement($this->replaceSpecialChars($pc->requirement));
					$new_pc_obj->setInstruction($this->replaceSpecialChars($pc->instruction));
