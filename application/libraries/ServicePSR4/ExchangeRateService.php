...
class ExchangeRateService extends BaseService
{
    ...
    {
        $this->setDao(new ExchangeRateDao);
        $this->setCurrencyDao(new CurrencyDao);
        $this->setExchangeRateApprovalDao(new ExchangeRateApprovalDao);
        $this->setExchangeRateHistoryDao(new ExchangeRateHistoryDao);
        $this->setTransmissionLogDao(new TransmissionLogDao);
        $this->setInterfaceExchangeRateDao(new InterfaceExchangeRateDao);
        $this->setBatchDao(new BatchDao);
        $this->setFtpInfoDao(new FtpInfoDao);
    ...
    public function setTransmissionLogDao($value)
    {
        $this->tlog_dao = $value;
    }
    ...
    public function getActiveCurrencyList()
    {
        ...
        $where = [];
        $option = [];
        $obj_array = $this->getCurrencyDao()->getActiveCurrencyList($where, $option);
    ...
    {
        return $this->getCurrencyDao()->getActiveCurrencyList($where = [], $option = []);
    }

    public function getCurrencyList($where = [], $option = [])
    {
        $rtn = [];
        $obj_array = $this->getCurrencyDao()->getList($where, $option);
        foreach ($obj_array as $obj) {
            $rtn[$obj->getCurrencyId()] = $obj->getName();
        }
    ...
    public function alterExchangeRate($from, $to, $rate, $dao)
    {
        ...
        if ($from != "" && $to != "") {
            $obj = $this->getSnapDao()->get();
            $obj->setFromCurrencyId($from);
            $obj->setToCurrencyId($to);
            $obj->setRate($rate);
            if ($dao == "ExchangeRateApprovalDao") {
                $obj->setApprovalStatus("0");
            } elseif ($dao == "ExchangeRateDao") {
                $app_obj = $this->getExchangeRateApprovalDao()->get();
                $app_obj->setFromCurrencyId($from);
                $app_obj->setToCurrencyId($to);
                $app_obj->setRate($rate);
                $app_obj->setApprovalStatus("1");
            }
            $num_row = $this->getSnapDao()->getNumRows(["from_currency_id" => $from, "to_currency_id" => $to]);
            if ($num_row) {
                $rtn = $this->getSnapDao()->update($obj);
                if ($rtn && $dao == "ExchangeRateDao") {
                    $rtn = $this->getExchangeRateApprovalDao()->update($app_obj);
                }
            } else {
                $rtn = $this->getSnapDao()->insert($obj);
                if ($rtn && $dao == "ExchangeRateDao") {
                    $rtn = $this->getExchangeRateApprovalDao()->insert($app_obj);
                }
            }
        } else {
            $rtn = FALSE;
    ...
    public function getBasedRate($base, $currency_list, $dao)
    {
        ...
        if ($base != "") {
            ...
            foreach ($currency_list as $key => $value) {
                if ($base != $key) {
                    $obj = $this->getSnapDao()->get(["from_currency_id" => $base, "to_currency_id" => $key]);
                    if (empty($obj)) {
                        $ret[$key] = 1.00;
                    } else {
                        $ret[$key] = $obj->getRate();
    ...
    {
        if ($from != "" && $to != "") {
            $ret = $this->getDao()->get(["from_currency_id" => $from, "to_currency_id" => $to]);
        } else {
            $ret = $this->getDao()->get();
        }
    ...
    {
        return $this->getExchangeRateApprovalDao()->getList($where, $option);
    }

    public function uploadExchangeRate()
    {
        $curr = "";
        $url = $this->contextConfigService->valueOf('xrate_url');
        $currency_list = $this->getCurrencyDao()->getList([]);
        $i = 0;
        ...
        foreach ($xrate AS $obj) {
            ...
            if (!($this->getExchangeRateHistoryDao()->get(["from_currency_id" => $data[0], "to_currency_id" => $data[1], "date" => $date["today"]]))) {
                $xrate_today = $this->getExchangeRateHistoryDao()->get();
                ...
                $this->getExchangeRateHistoryDao()->insert($xrate_today);
            }
            $xrate_system = $this->getDao()->get(["from_currency_id" => $data[0], "to_currency_id" => $data[1]]);
            if ($xrate_system) {
            ...
            if ($difference) {
                ...
                $xrate = $this->getDao()->get(["from_currency_id" => $data[0], "to_currency_id" => $data[1]]);
                if ($xrate) {
                    $xrate->setRate($data[4]);
                } else {
//                  var_dump(__LINE__.$this->db->last_query());
                }
                $this->getDao()->update($xrate);
            }
            $xrate_obj = $this->getExchangeRateApprovalDao()->get(["from_currency_id" => $data[0], "to_currency_id" => $data[1]]);
            if ($xrate_obj) {
                ...
                $this->getExchangeRateApprovalDao()->update($xrate_obj);
            } else {
                $xrate_obj = $this->getExchangeRateApprovalDao()->get();
                ...
                $this->getExchangeRateApprovalDao()->insert($xrate_obj);
            }
        }
        $email_to = $this->contextConfigService->valueOf('alan_email');
        if ($remark) {
    ...
    public function updateExchangeRateFromCv()
    {
        if ($filename = $this->getExchangeRateFile()) {
            ...
            if ($rs = $this->validateExchangeRateFile($filename)) {
                ...
                $this->getBatchDao()->insert($batch_obj);
            } else {
            ...
            $batch_obj = $this->getBatchDao()->get(["remark" => $filename]);
            $batch_id = $batch_obj->getId();

            $iex_vo = $this->getInterfaceExchangeRateDao()->get();
            foreach ($rs AS $row) {
                ...
                $this->getInterfaceExchangeRateDao()->insert($iex_obj);
            }
    ...
    {
        $local_path = $this->contextConfigService->valueOf("ex_rate_data_path");
        $ftp = $this->ftpConnector;
        $ftpObj = $this->getFtpInfoDao()->get(["name" => "CV_EXCHANGE_RATE"]);
        ...
        $remote_path = "/";
        $dao = $this->getDao();
        $tlog_vo = $this->getTransmissionLogDao()->get();
        ...
        if ($ftp->connect() !== FALSE) {
            if ($ftp->login() !== FALSE) {
                if ($ftp->getfile($local_path . $filename, $remote_path . $filename)) {
                    ...
                    $this->getTransmissionLogDao()->insert($tlog_obj);
                }
            } else {
                $tlog_obj = clone $tlog_vo;
                $tlog_obj->setFuncName($func);
    ...
    public function getTransmissionLogDao()
    {
    ...
    public function validateExchangeRateFile($filename)
    {
        ...
        $local_path = $this->contextConfigService->valueOf("ex_rate_data_path");
        $tlog_obj = $this->getTransmissionLogDao()->get();
    ...
    public function batchExchangeRate($batch_id, $data)
    {
        ...
        set_time_limit(180);
        $success = 1;
        if ($batch_obj = $this->getBatchDao()->get(["id" => $batch_id])) {
            $objlist = $this->getInterfaceExchangeRateDao()->getList(["batch_id" => $batch_id, "batch_status" => "N"], ["limit" => -1]);
            if ($objlist) {
                foreach ($objlist AS $iex_obj) {
                    ...
                    if ($rs["valid"]) {
                        if ($ex_rate_obj = $this->getDao()->get(["from_currency_id" => $iex_obj->getFromCurrencyId(), "to_currency_id" => $iex_obj->getToCurrencyId()])) {
                    ...
                    $this->getInterfaceExchangeRateDao()->update($iex_obj);
                }
            }
        }
        ...
        if (!$success) {
            ...
            $this->getBatchDao()->update($batch_obj);
        }
    ...
    public function proceedExchangeRate($batch_id)
    {
        ...
        $batch_err = 0;
        $err_msg = "";
        $batch_obj = $this->getBatchDao()->get(["id" => $batch_id]);
        if ($batch_obj) {
            $batch_obj->setStatus("P");
            $this->getBatchDao()->update($batch_obj);

            $iex_list = $this->getInterfaceExchangeRateDao()->getList(["batch_id" => $batch_id, "batch_status" => "R"], ["limit" => -1]);
            if (!empty($iex_list)) {
                $ex_vo = $this->getDao()->get();
                foreach ($iex_list as $iex_obj) {
                    if (!($this->getExchangeRateHistoryDao()->get(["from_currency_id" => $iex_obj->getFromCurrencyId(), "to_currency_id" => $iex_obj->getToCurrencyId(), "date" => date("Ymd 00:00:00")]))) {
                        $xrate_today = $this->getExchangeRateHistoryDao()->get();
                    ...
                    if ($ex_obj = $this->getDao()->get(["from_currency_id" => $iex_obj->getFromCurrencyId(), "to_currency_id" => $iex_obj->getToCurrencyId()])) {
                        $action = "update";
                    } else {
                        $action = "insert";
                        $ex_obj = clone $ex_vo;
                    ...
                    $ret = $this->getDao()->$action($ex_obj);
                    if ($ret === FALSE) {
                        $iex_obj->setBatchStatus('F');
                        $iex_obj->setFailedReason($this->getDao()->db->_error_message());
                        $this->getInterfaceExchangeRateDao()->update($iex_obj);

                        var_dump($this->getDao()->db->_error_message());
                        $err_msg .= "LINE: " . __LINE__ . "\nREASON: " . $this->getDao()->db->_error_message() . "\nSQL: " . $this->getDao()->db->last_query() . "\n\n\n";
                        $batch_err = 1;
//                      break;
                    } else {
                        $iex_obj->setBatchStatus('S');
    ...
    {
        set_time_limit(180);
        $approval_obj = $this->getExchangeRateApprovalDao()->get(["from_currency_id" => $ex_obj->getFromCurrencyId(), "to_currency_id" => $ex_obj->getToCurrencyId()]);
        if ($approval_obj) {
            $approval_obj->setRate($ex_obj->getRate());
            $approval_obj->setApprovalStatus("1");
            $ret = $this->getExchangeRateApprovalDao()->update($approval_obj);
        } else {
            $approval_obj = $this->getExchangeRateApprovalDao()->get();
            $approval_obj->setFromCurrencyId($ex_obj->getFromCurrencyId());
            $approval_obj->setToCurrencyId($ex_obj->getToCurrencyId());
            $approval_obj->setRate($ex_obj->getRate());
            $approval_obj->setApprovalStatus("1");
            $ret = $this->getExchangeRateApprovalDao()->insert($approval_obj);
        }
        if ($ret === FALSE) {
    ...
    {
        $xrate_obj = $this->getDao()->get(["from_currency_id" => $from, "to_currency_id" => $to]);
    ...
    public function getExchangeRateApprovalDao()
    {
        return $this->exchangeRateApprovalDao;
    }

    public function setExchangeRateApprovalDao($value)
    {
        $this->exchangeRateApprovalDao = $value;
    }

    public function getExchangeRateHistoryDao()
    {
        return $this->exchangeRateHistoryDao;
    }

    public function setExchangeRateHistoryDao($value)
    {
        $this->exchangeRateHistoryDao = $value;
    }

    public function getInterfaceExchangeRateDao()
    {
        return $this->iex_dao;
    }

    public function setInterfaceExchangeRateDao($value)
    {
        $this->iex_dao = $value;
    }

    public function getSnapDao()
    {
        return $this->snapDao;
    }

    public function setSnapDao($value)
    {
        $this->snapDao = $value;
    }

    public function getFtpInfoDao()
    {
        return $this->ftpInfoDao;
    }

    public function setFtpInfoDao($value)
    {
        $this->ftpInfoDao = $value;
    }
    public function setBatchDao($value)
    {
        $this->batch_dao = $value;
    }

    public function getBatchDao()
    {
