...
class RefundService extends BaseService
{
    ...
    public function quick_refund($so_no, $amount, $notes)
    {
        ...
        $this->getDao('Refund')->trans_start();
                ...
                if ($refund_item_result !== FALSE) {
                    ...
                    if ($history_result !== FALSE) {
                        ...
                        if ($history_result !== FALSE) {
                            ...
                            if ($so_result !== FALSE) {
                                $this->getDao('Refund')->trans_complete();
        ...
        if ($rollback) {
            $this->getDao('Refund')->trans_rollback();
        }
        return false;
    }
    ...
    public function check_action($refundid = "", $action = "", $auto_refund = false)
    {
        ...
        $result = $this->getDao('Refund')->check_complete($refundid);
        if ($result === FALSE) {
            return $result;
        } else if ($result["total"] == $result["completed"]) {

            $refund_obj = $this->getDao('Refund')->get(array("id" => $refundid));
            if ($refund_obj === FALSE) {
                return $refund_obj;
            }
    ...
    public function fireEmail($rid, $status, $type, $from = "")
    {
        if ($status == "" || $type == "" || $rid == "") {
            ...
            $robj = $this->getDao('Refund')->get(array("id" => $rid));
            $user_obj = $this->getDao('User')->get(["id" => $robj->get_create_by()]);
