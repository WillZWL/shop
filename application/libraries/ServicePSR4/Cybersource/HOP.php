...
{
    return "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDD33DareKWADYC3wZflsVv5uXrryfhrgCX5G5jYLtFgiGLcI6TeWZ/abbcwLzaOUruW+qbjg68pIdWZ868ixsHMUqMV1oasKPzg3lHubaj3WxUm5VS0BIslfLkidiwHlezH9HlPRAFW+qamlo1lrEgO+/4M1tB4+1FeQdNabv2YQIDAQAB";
}

function getSerialNumber()
{
    return "3466560486080176056166";
}
...
function InsertSignature3($amount, $currency, $orderPage_transactionType, $accountDetails = array())
{
    if (!isset($amount)) {
        $amount = "0.00";
    }
    ...
    $merchantID = $accountDetails["merchantId"];
    $timestamp = getmicrotime();
    $data = $merchantID . $amount . $currency . $timestamp . $orderPage_transactionType;
    $pub = $accountDetails["secret"];
    $serialNumber = $accountDetails["serialNumber"];
    $pub_digest = hopHash($data, $pub);

    $necessary_data = array();
    $necessary_data['orderPage_transactionType'] = $orderPage_transactionType;
    $necessary_data['orderPage_timestamp'] = $timestamp;
    $necessary_data['merchantID'] = $merchantID;
    $necessary_data['orderPage_signaturePublic'] = $pub_digest;
    $necessary_data['orderPage_version'] = '7';
    $necessary_data['orderPage_serialNumber'] = $serialNumber;
...
{
    if (!isset($subscriptionFrequency)) {
        return;
    }
    if (!isset($subscriptionAmount)) {
        $subscriptionAmount = "0.00";
    }
    if (!isset($subscriptionStartDate)) {
        $subscriptionStartDate = "00000000";
    }
    if (!isset($subscriptionNumberOfPayments)) {
        $subscriptionNumberOfPayments = "0";
    }
    if (!isset($subscriptionAutomaticRenew)) {
