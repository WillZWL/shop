...
class ReviewFianetService extends BaseService
{
    ...
    public function sendOrderData($so_obj, $client_obj)
    {
        ...
        if ($so_obj && $client_obj) {
            ...
            if (in_array($platform_id, $allow_platform_id) && in_array($biz_type, $allow_biz_type)) {
                ...
                $billname = $so_obj->getBillName();
                $ipaddress = $so_obj->getCreateAt();
                $order_create_date = $so_obj->getOrderCreateDate();
                $totalamount = $so_obj->getAmount();
                ...
                if ($xml = $dom->saveXML()) {
                    ...
                    if (empty($error)) {
                        ...
                        if ($response_xml = simplexml_load_string($content, 'SimpleXMLElement', LIBXML_NOCDATA)) {
                            ...
                            if (strtoupper($result[0]) == 'OK') {
                                ...
                                $this->sendNotificationEmail("response", $message);
                            }
                        }
                    } else {
                        ...
                        foreach ($errors as $error) {
                            $libxml_errors .= $this->displayXmlError($error, $xml);
                        }
                    ...
                    $this->sendNotificationEmail("xml", $message);
                }
            }
        } else {
            ...
            $this->sendNotificationEmail("nodata", $message);
        }
    ...
    private function sendNotificationEmail($type = "", $message = "")
    {
