...
class FaqadminService extends BaseService
{
    ...
    {
        $pbv_svc = $this->platformBizVarService;

        $pbv_obj = $pbv_svc->getDao()->get(array("selling_platform_id" => $platform_id));
        if (!$pbv_obj) {
            $lang_id = 'en';
        } else {
        ...
        $faq_obj = $this->get(["lang_id" => $lang_id]);
        if (!$faq_obj) {
            $faq_ver = "cveng";
        } else {
            $faq_ver = $faq_obj->getFaqVer();
        }

        $hash_str = ["cveng" => "a922f5e654fb5bcbdc18cdfb30b9a6ef", "cv-fr" => "7710e562acfc94706c5ed32776a749bb", "cv-de" => "175508585e967bcb4ad502426032b4f2", "cv-es" => "4381adf386419dc4d7f2d836a03f59e1"];
        $domain = ["cveng" => "http://cveng.host4kb.com/", "cv-fr" => "http://cv-fr.host4kb.com/", "cv-de" => "http://cv-de.host4kb.com/", "cv-es" => "http://cv-es.host4kb.com/"];
