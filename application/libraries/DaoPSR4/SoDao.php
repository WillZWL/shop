...
SQL;
    ...
    public function getDispatchData($where = [])
    {
        $where_clause = "";
        $where_clause2 = 'where so.dispatch_date > "' . $where['from_date'] . ' 00:00:00" and so.dispatch_date < "' . $where['to_date'] . ' 23:59:59' . '"';
        ...
        $sql = "SELECT
                ...
                $where_clause2
                $where_clause
                order by soid.so_no,soid.amount desc";
        $result = $this->db->query($sql);
        if (!$result) {
            return FALSE;
        }
...
SQL;
    ...
    public function getAftershipReportForFtp($where = [], $option = [], $classname = "AftershipDataDto")
    {
        $option['limit'] = -1;
        $this->db->from("so");
        $this->db->join("so_allocate soa", "soa.so_no = so.so_no AND line_no = 1", "INNER");
        $this->db->join("client c", "c.id = so.client_id", "INNER");
        $this->db->join("so_shipment sosh", "soa.sh_no = sosh.sh_no", "INNER");
        $this->db->join("courier cr", "cr.courier_id = sosh.courier_id", "INNER");
...
SQL;
    ...
    public function getSalesOrder($where = [], $option = [], $classname = 'OrderNotInRiaReportDto')
    {
        ...
        $select_str = 'so.so_no,
                       ...
                       so.order_create_date,
                       concat_ws(" ", CAST(c.title as CHAR), CAST(c.forename as CHAR), CAST(c.surname as CHAR)) as name,
                       so.delivery_address,
                       so.delivery_postcode,
                       so.delivery_city,
                       so.delivery_state,
    ...
    public function getRiaOrder($where = array(), $option = array(), $classname = 'riaControlDto')
    {
        ...
        $where["((rf.modify_on >= '$end_date') OR (rf.status is NUll))"] = NULL;
