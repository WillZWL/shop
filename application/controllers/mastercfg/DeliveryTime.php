...
class DeliveryTime extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data["scenario_list"] = $this->sc['DeliveryTime']->getDeliveryScenarioList();
        $data["country_list"] = $this->sc['Country']->getSellCountryList();
        $del_list = $this->sc['DeliveryTime']->getDeliverytimeList();
    ...
    private function update_form($postdata = [])
    {
        $ret = [];
        $ret["status"] = FALSE;
        $deliverytimeDao = $this->sc['DeliveryTime']->getDao('DeliveryTime');
        ...
        if ($postdata) {
            ...
            if ($scenario_list = $this->sc['DeliveryTime']->getDeliveryScenarioList()) {
            ...
            foreach ($postdata as $ctry_id => $value) {
                $data_exists = false;
                if (($check_empty = $this->sc['DeliveryTime']->checkEmptyFields($value)) === false) {
                    $success = 0;
            ...
            if ($email_msg) {
                ...
                $this->sc['DeliveryTime']->sendNotificationEmail("CHG", $email_msg);
            }
        } else {
