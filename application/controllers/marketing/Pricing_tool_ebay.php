...
class Pricing_tool_ebay extends MY_Controller
{

    private $app_id = 'MKT0051';
    private $lang_id = 'en';
            ...
            if($this->input->post('posted'))
            {
                ...
                foreach($plat as $key=>$val)
                {
                    ...
                    if (call_user_func(array($price_ext_obj, "get_platform_id")))
                    {
                        ...
                        if ($price_ext_obj->get_ext_ref_1() != $_POST["price_ext"][$key]["ext_ref_1"] ||
                            ...
                            $price_ext_obj->get_handling_time() != $_POST["price_ext"][$key]["handling_time"] ||
                            $_POST["action"][$key]
                            )
                        {
                        ...
                        )
                    {
                        ...
                        else
                        {
                            ...
                            if ($price_ext_need_update)
                            {
                                ...
                                elseif ($_POST["action"][$key] == "E")
                                {
                                    $price_ext_obj->set_action("E");
                                    $price_ext_obj->set_remark($_POST["reason"][$key]);
                                }
                                ...
                                else
                                {
                                    ...
                                    {
                                        $res = $this->ebay_service->end_item($key, $value);
                                        if($res["response"])
                                        {
                                    ...
                                    elseif ($_POST["action"][$key] == "RE")
                                    {
                                        $res = $this->ebay_service->revise_item($key, $value);
                                        $_SESSION["NOTICE"] .= $res["message"];
                                    }
                                }
                            }
            ...
            if($value != "")
            {
                if ($platform_list = $this->pricing_tool_model->platform_biz_var_service->get_pricing_tool_platform_list($value, PLATFORM_TYPE))
                {
                    foreach($platform_list as $platform_obj)
                    {
                        ...
                        //#3890 set default handling time
                        if(!$price_ext_obj->get_handling_time())
                        {
                            switch(strtoupper($platform_id))
                            {
                                case 'EBAYAU':
                                case 'EBAYSG':
                            ...
                            $price_ext_obj->set_handling_time($default_handling_time);
                        }
                        ...
                        // eBay Store Category
                        $data["store_cat_list"][$platform_obj->get_selling_platform_id()] = $this->product_model->external_category_service->get_list(array("ext_party"=>"EBAY_STORE", "level"=>1, "country_id"=>$platform_obj->get_platform_country_id(), "status"=>1), array("orderby"=>"ext_name ASC", "limit"=>-1));
                        $data["store_cat_2_list"][$platform_obj->get_selling_platform_id()] = $this->product_model->external_category_service->get_list(array("ext_party"=>"EBAY_STORE", "level"=>2, "country_id"=>$platform_obj->get_platform_country_id(), "status"=>1), array("orderby"=>"ext_name ASC", "limit"=>-1));
                    }
                }
                $data["pdata"] = $pdata;
    ...
    public function get_profit_margin_json($platform_id, $sku, $required_selling_price = 0, $required_cost_price = -1, $platform_type="WEBSITE")
    {
