...
class Product_overview_amfr extends MY_Controller
{
	...
	{
		parent::__construct();
		$this->load->model('marketing/product_overview_amfr_model');
	...
	public function index($platform_id="AMFR")
	{
		...
		switch ($platform_id)
		{
			default: $price_service = "amfr_price";
					break;
		}
		...
		if ($this->input->post("posted") && $_POST["check"])
		{
			...
			{
				$success = 0;
				if (($price_obj = $this->product_overview_amfr_model->get_price($price_service, array("sku"=>$rssku, "platform_id"=>$platform_id)))!==FALSE)
				{
					if (empty($price_obj))
					{
						$price_obj = $this->product_overview_amfr_model->get_price($price_service);
						...
						if ($this->product_overview_amfr_model->add_price($price_service, $price_obj))
						{
							$success = 1;
						}
					}
					...
					{
						set_value($price_obj, $_POST["price"][$rssku]);
						if ($this->product_overview_amfr_model->update_price($price_service, $price_obj))
						{
							$success = 1;
						}
					}
				...
				if ($success)
				{
					if ($product_obj = $this->product_overview_amfr_model->get("product", array("sku"=>$rssku)))
					{
						set_value($product_obj, $_POST["product"][$rssku]);
						if ($this->product_overview_amfr_model->update("product", $product_obj))
						{
							$success = 1;
						}
						else
		...
		{
			$data["objlist"] = $this->product_overview_amfr_model->get_product_list($where, $option, $lang);
			$data["total"] = $this->product_overview_amfr_model->get_product_list_total($where, $option);
		}
		...
		$js = "function CalcProfit(sku, price)
			{
				...
				var freight_cost = prod[sku]['freight_cost']*1;
				...
				var vat = (declared + delivery_charge*1) * vat_pcent  / (100 + vat_pcent);
				vat = vat.toFixed(2)*1;
