...
class Pricing_tool_amuk extends MY_Controller
{

	private $app_id = 'MKT0009';
	private $lang_id = 'en';
	...
	public function __construct()
	{
		...
		$this->load->model('marketing/pricing_tool_amuk_model');
	...
	public function index()
	{
		...
		$data["lang"] = $lang;
		$this->load->view("marketing/pricing_tool_amuk/pricing_tool_amuk_index",$data);
	}
	...
	public function plist()
	{
		...
		if (($sku = $this->input->get("sku")) != "" || ($prod_name = $this->input->get("name")) != "")
		{
			...
			$data["objlist"] = $this->pricing_tool_amuk_model->get_product_list($where, $option);
			$data["total"] = $this->pricing_tool_amuk_model->get_product_list_total($where);
		...
		$this->load->view('marketing/pricing_tool_amuk/pricing_tool_amuk_list', $data);
	}
	...
	public function view($value="")
	{
		...
		if($no_of_valid_supplier == 1)
		{
			...
			{
				//print_r($_POST);
				$this->pricing_tool_amuk_model->__autoload();
				$price_obj = unserialize($_SESSION["price_obj"]);
				$price_obj->set_platform_id("AMUK");
				...
				{
					$ret = $this->pricing_tool_amuk_model->update($price_obj);
				}
				else
				{
					$ret = $this->pricing_tool_amuk_model->add($price_obj);
				}
				...
				$this->pricing_tool_amuk_model->__autoload_product_vo();
				...
				$prod_obj->set_freight_cat_id($this->input->post('freight_cat_id'));
				...
				$ret2 = $this->pricing_tool_amuk_model->update_product($prod_obj);
				if($ret2 === FALSE)
				{
					$_SESSION["NOTICE"] = $this->db->_error_message();
				}
				...
				{
					$note_obj = $this->pricing_tool_amuk_model->get_note();
					$note_obj->set_platform_id("AMUK");
					$note_obj->set_sku($value);
					$note_obj->set_type('M');
					$note_obj->set_note($this->input->post('m_note'));
					if (!($ret3 = $this->pricing_tool_amuk_model->add_note($note_obj)))
					{
				...
				if(trim($this->input->post('s_note')) != "")
				{
					...
					if (!($ret4 = $this->pricing_tool_amuk_model->add_note($note_obj)))
					{
						$_SESSION["NOTICE"] = "update_note_failed";
					}
				}
				Redirect(base_url()."marketing/pricing_tool_amuk/view/".$value);
			}

			$shiptype_list = $this->pricing_tool_amuk_model->get_shiptype_list();
			$price_obj = $this->pricing_tool_amuk_model->get_price_obj(array("platform_id"=>"AMUK","sku"=>$value));
			$data["action"] = "update";
			if(empty($price_obj))
			{
				$price_obj = $this->pricing_tool_amuk_model->get_price_obj();
				$data["action"] = "add";
			}
			...
			$data["inv"] = $this->pricing_tool_amuk_model->get_inventory(array("sku"=>$value));

			$tmpx = $this->pricing_tool_amuk_model->get_quantity_in_orders($value);
			...
			$product_cost_dto = $this->pricing_tool_amuk_model->get_product_cost_dto($value,"AMUK");
			...
			if($value != "")
			{
				...
				foreach($product_cost_dto as $obj)
				{
					...
					$this->pricing_tool_amuk_model->set_dto_ps($obj);
					$this->pricing_tool_amuk_model->calc_profit_ps();

					$data["table_row"] .= $this->pricing_tool_amuk_model->get_table_row_ps($price_obj->get_default_shiptype());
					$data["objcount"]++;
				}

				$prod_obj =  $this->pricing_tool_amuk_model->get_prod($value);

				if ($data["objcount"])
				{
					$data["table_header"] = $this->pricing_tool_amuk_model->get_table_header_ps();
					//$data["js"] = $this->pricing_tool_amuk_model->get_js_ps();
					$data["shiptype"] = $shiptype;
					$currency = $this->pricing_tool_amuk_model->get_currency();
					$data["currency_obj"] = $this->pricing_tool_amuk_model->get_currency_detail($currency);
					$data["mkt_note_obj"] = $this->pricing_tool_amuk_model->get_note($value, "M");
					$data["src_note_obj"] = $this->pricing_tool_amuk_model->get_note($value, "S");
				}
			}

			if($value != "")
			{
				$data["supplier"] = $this->pricing_tool_amuk_model->get_current_supplier($value);
	//			$fcat = $this->pricing_tool_amuk_model->get_freight_cat($prod_obj->get_freight_cat_id());
	//			$data["freight_cat"] = $fcat->get_name();
		...
		$this->load->view("marketing/pricing_tool_amuk/pricing_tool_amuk_view",$data);
		...
		$js ="	function rePrice(typeInput)
				{
					...
					for(var i =0; i <shiptype.length;i++)
					{
						...
						freight_cost = type==2?'0.00':freight_cost.toFixed(2);
						var declared = document.getElementById('declared_rate['+type+']').value * price / 100 * 1;
						declared = type==2?'0.00':declared.toFixed(2);
						...
						var vat = (declared * 1 + admin_fee * 1) * vat_pcent/100;
						vat = type==2?'0.00':vat.toFixed(2);
