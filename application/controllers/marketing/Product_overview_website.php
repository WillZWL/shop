...
class Product_overview_website extends MY_Controller
{
    ...
    public function upload_clearance_sku_info()
    {
        ...
        $receipient = "nero@eservicesgroup.com, celine@eservicesgroup.com, bd_product_team@eservicesgroup.com, perry.leung@eservicesgroup.com";
    ...
    private function process_sku_info_file($filename)
    {
        ...
        foreach ($arr as $line)
        {
            ...
            {
                case "":
                    $output = "SUCCESS: {$line[0]}'s {$line[3]} {$line[1]} ({$line[2]}) to be priced at {$line[4]}, margin is {$json["get_margin"]}, recommend to sell at $auto_price<br>\r\n";
    ...
    public function index($platform_id = "")
    {
        ...
        if ($this->input->post("posted") && $_POST["check"])
        {
            ...
            foreach ($_POST["check"] as $rssku)
            {
                ...
                if (($price_obj = $this->product_overview_model->get_price(array("sku"=>$sku, "platform_id"=>$platform)))!==FALSE)
                {
                    ...
                    {
                        set_value($price_obj, $_POST["price"][$platform][$sku]);
                        if($_POST["price"][$platform][$sku]["auto_price"] == 'on')
                        {
        ...
        {
            $where["map.ext_sku LIKE "] = "%".$this->input->get("master_sku")."%";
            $submit_search = 1;
        }

        if ($this->input->get("cat_id") != "")
        {
            $where["v_prod_overview_wo_shiptype.cat_id"] = $this->input->get("cat_id");
        }

        if ($this->input->get("sub_cat_id") != "")
        {
            $where["v_prod_overview_wo_shiptype.sub_cat_id"] = $this->input->get("sub_cat_id");
        }

        if ($this->input->get("brand_id") != "")
        {
            $where["v_prod_overview_wo_shiptype.brand_id"] = $this->input->get("brand_id");
        }

        if ($this->input->get("supplier_id") != "")
        {
            $where["v_prod_overview_wo_shiptype.supplier_id"] = $this->input->get("supplier_id");
        }
        ...
        {
            $condition = "v_prod_overview_wo_shiptype.platform_id IN ('";
        ...
        {
            $where["v_prod_overview_wo_shiptype.clearance"] = $this->input->get("clearance");
            $submit_search = 1;
        }
        ...
        {
            $where["v_prod_overview_wo_shiptype.website_status"] = $this->input->get("website_status");
            $submit_search = 1;
        }
        ...
        if ($this->input->get("surplus_quantity") != "")
        {
            switch($this->input->get("surplus_quantity_prefix"))
            {
                case 1:
                    $where["surplus_quantity is not null and surplus_quantity > 0 and surplus_quantity <= {$this->input->get("surplus_quantity")}"] = null;
                    break;
                case 2:
                    $where["surplus_quantity <= {$this->input->get("surplus_quantity")}"] = null;
                    break;
                case 3:
                    $where["surplus_quantity >= {$this->input->get("surplus_quantity")}"] = null;
                    break;
        ...
        if ($this->input->get("affiliate_feed") != "")
        {
            $affiliate_feed = $option["affiliate_feed"] = $this->input->get("affiliate_feed");
        ...
        if ($this->input->get("search"))
        {
            if ($this->input->get("csv"))
            {
                ...
                {
                    $where["listing_status"] = $where["pr.listing_status"];
                ...
                $list = $this->product_overview_model->get_product_overview($where, array_merge($option));
        ...
        $data["affiliate_feed"] = $affiliate_feed;
