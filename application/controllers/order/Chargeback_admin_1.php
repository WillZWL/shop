...
class Chargeback_admin_1 extends gridpage
{
    ...
    function setup_columns()
    {
        ...
        $objGrid->FormatColumn("chargeback_reason","Reason",        "0", "50", 1, "20", "left", "text");

        $objGrid->FormatColumn("chargeback_remark_id","Remarks",    "0", "50", 1, "15", "left", "selected:select * from lookup_chargeback_remark");
        $objGrid->FormatColumn("chargeback_remark","Order Notes",   "0", "50", 1, "20", "left", "text");
    ...
    function execute_custom_sql()
    {
        ...
        "
            select
                ...
                #pp.prod_name
            from so
            inner join so_item_detail soid           on soid.so_no = so.so_no and so.status > 2
            inner join sku_mapping sm       on sm.sku = soid.item_sku and sm.status = 1 and sm.ext_sys = 'wms'
            inner join price px             on px.sku = soid.item_sku and px.platform_id = so.platform_id
            inner join product pp           on pp.sku = soid.item_sku
        ...
        foreach ($_POST as $k=>$v)
        {
            if (!empty($v))
            {
                ...
                switch ($kk)
                {
                    ...
                    // local sku
                    case 6:     $query .= " and soid.item_sku = '$v' ";               break;

                    case 8:     $v = str_replace(" ", "%", $v);
                                $query .= " and soid.prod_name like '%$v%' ";         break;
