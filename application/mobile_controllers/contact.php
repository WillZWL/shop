...
class Contact extends MOBILE_Controller
{
	protected $data;
	...
	{
		$this->data = array_merge($this->data, $this->common_data_prepare_model->get_data_array($this));

		$arr_phone_contact_country = array('HK', 'FI', 'IE', 'AU', 'US', 'GB');
		if (in_array(strtoupper(PLATFORMCOUNTRYID), $arr_phone_contact_country))
		{
			$data['phone_contact'] = TRUE;
		}
		else
		{
			$data['phone_contact'] = FALSE;
		}

		$this->template->add_js('/js/checkform.js');
		$this->load_tpl('content', 'contact', $data, TRUE, TRUE);
	}
